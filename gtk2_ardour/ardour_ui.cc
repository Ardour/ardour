	...
	ARDOUR::PresentationInfo::order_t order)
{
	...
	try {
		...
		if (tracks.size() != how_many) {
			error << string_compose (P_("could not create %1 new trigger track", "could not create %1 new trigger tracks", how_many), how_many) << endmsg;
		}
	}
...
ARDOUR_UI::add_route_dialog_response (int r)
{
	...
	string template_path = add_route_dialog->get_template_path();
	DisplaySuspender ds;
