 ...
 */
AutomationLineBase::AutomationLineBase (const string&                   name,
                                        EditingContext&                 ec,
...
AutomationLineBase::~AutomationLineBase ()
{
...
timepos_t
AutomationLineBase::get_origin() const
{
...
bool
AutomationLineBase::is_stepped() const
{
	return (_desc.toggled || (alist && alist->interpolation() == AutomationList::Discrete));
}

void
AutomationLineBase::update_visibility ()
{
	if (_visible & Line) {
...
bool
AutomationLineBase::get_uses_gain_mapping () const
{
	switch (_desc.type) {
		case GainAutomation:
		case BusSendLevel:
...
void
AutomationLineBase::hide ()
{
	/* leave control points setting unchanged, we are just hiding the
	   overall line
	*/

	set_visibility (AutomationLineBase::VisibleAspects (_visible & ~Line));
}

double
AutomationLineBase::control_point_box_size ()
{
...
void
AutomationLineBase::set_height (guint32 h)
{
	if (h != _height) {
		_height = h;
...
void
AutomationLineBase::set_line_color (string color_name, std::string color_mod)
{
...
uint32_t
AutomationLineBase::get_line_color() const
{
	return UIConfiguration::instance().color (_line_color);
}

ControlPoint*
AutomationLineBase::nth (uint32_t n)
{
...
ControlPoint const *
AutomationLineBase::nth (uint32_t n) const
{
	if (n < control_points.size()) {
		return control_points[n];
	} else {
...
void
AutomationLineBase::modify_points_y (std::vector<ControlPoint*> const& cps, double y)
{
...
void
AutomationLineBase::reset_line_coords (ControlPoint& cp)
{
...
bool
AutomationLineBase::sync_model_with_view_points (list<ControlPoint*> cp)
{
	update_pending = true;
...
string
AutomationLineBase::get_verbose_cursor_string (double fraction) const
{
	return fraction_to_string (fraction);
}

string
AutomationLineBase::get_verbose_cursor_relative_string (double fraction, double delta) const
{
 ...
 */
string
AutomationLineBase::fraction_to_string (double fraction) const
{
...
string
AutomationLineBase::delta_to_string (double delta) const
{
 ...
 */
double
AutomationLineBase::string_to_fraction (string const & s) const
{
	double v;
	sscanf (s.c_str(), "%lf", &v);
...
AutomationLineBase::ContiguousControlPoints::ContiguousControlPoints (AutomationLineBase& al)
	: line (al), before_x (timepos_t (line.the_list()->time_domain())), after_x (timepos_t::max (line.the_list()->time_domain()))
{
}

void
AutomationLineBase::ContiguousControlPoints::compute_x_bounds ()
{
	uint32_t sz = size();
...
Temporal::timecnt_t
AutomationLineBase::ContiguousControlPoints::clamp_dt (timecnt_t const & dt, timepos_t const & line_limit)
{
	if (empty()) {
		return dt;
	}
...
void
AutomationLineBase::ContiguousControlPoints::move (timecnt_t const & dt, double dvalue)
{
	for (auto & cp : *this) {
 ...
 */
void
AutomationLineBase::start_drag_common (double x, float fraction)
{
 ...
 */
double
AutomationLineBase::dt_to_dx (timepos_t const & pos, timecnt_t const & dt)
{
 ...
 */
pair<float, float>
AutomationLineBase::drag_motion (timecnt_t const & pdt, float fraction, bool ignore_x, bool with_push, uint32_t& final_index)
{
...
void
AutomationLineBase::end_drag (bool with_push, uint32_t final_index)
{
	if (!_drag_had_movement) {
		return;
	}
 ...
 */
bool
AutomationLineBase::sync_model_with_view_point (ControlPoint& cp)
{
...
bool
AutomationLineBase::control_points_adjacent (double xval, uint32_t & before, uint32_t& after)
{
	ControlPoint *bcp = 0;
	ControlPoint *acp = 0;
	double unit_xval;
...
bool
AutomationLineBase::is_last_point (ControlPoint& cp)
{
...
bool
AutomationLineBase::is_first_point (ControlPoint& cp)
{
 ...
 */
void
AutomationLineBase::get_selectables (timepos_t const & start, timepos_t const & end, double botfrac, double topfrac, list<Selectable*>& results)
{
...
void
AutomationLineBase::get_inverted_selectables (Selection&, list<Selectable*>& /*results*/)
{
	// hmmm ....
}

void
AutomationLineBase::set_selected_points (PointSelection const & points)
{
	for (vector<ControlPoint*>::iterator i = control_points.begin(); i != control_points.end(); ++i) {
...
void
AutomationLineBase::list_changed ()
{
	DEBUG_TRACE (DEBUG::Automation, string_compose ("\tline changed, existing update pending? %1\n", update_pending));

	if (!update_pending) {
		update_pending = true;
		Gtkmm2ext::UI::instance()->call_slot (invalidator (*this), boost::bind (&AutomationLineBase::queue_reset, this));
	}
}

void
AutomationLineBase::tempo_map_changed ()
{
...
void
AutomationLineBase::reset_callback (const Evoral::ControlList& events)
{
	uint32_t vp = 0;
	uint32_t pi = 0;
	uint32_t np;
...
AutomationLineBase::reset ()
{
	 ...
	 * (the control-point model, used by AutomationLineBase::drag_motion, will be invalid).
	 *
	...
	alist->apply_to_points (*this, &AutomationLineBase::reset_callback);
}

void
AutomationLineBase::queue_reset ()
{
	...
	if (editing_context.session()->transport_rolling() && alist->automation_write()) {
		...
		if (!have_reset_timeout) {
			...
			Glib::signal_timeout().connect (sigc::bind_return (sigc::mem_fun (*this, &AutomationLineBase::reset), false), 250);
...
void
AutomationLineBase::set_list (std::shared_ptr<ARDOUR::AutomationList> list)
{
	alist = list;
	queue_reset ();
	connect_to_list ();
...
void
AutomationLineBase::add_visibility (VisibleAspects va)
{
...
void
AutomationLineBase::set_visibility (VisibleAspects va)
{
	if (_visible != va) {
		_visible = va;
		update_visibility ();
...
void
AutomationLineBase::remove_visibility (VisibleAspects va)
{
	VisibleAspects old = _visible;
...
void
AutomationLineBase::track_entered()
{
	add_visibility (ControlPoints);
}

void
AutomationLineBase::track_exited()
{
	remove_visibility (ControlPoints);
}

XMLNode &
AutomationLineBase::get_state () const
{
...
int
AutomationLineBase::set_state (const XMLNode &node, int version)
{
	/* function as a proxy for the model */
...
void
AutomationLineBase::view_to_model_coord_y (double& y) const
{
...
double
AutomationLineBase::compute_delta (double from, double to) const
{
	return _desc.compute_delta (from, to);
}

void
AutomationLineBase::apply_delta (double& val, double delta) const
{
...
double
AutomationLineBase::model_to_view_coord_y (double y) const
{
	if (alist->default_interpolation () != alist->interpolation()) {
		switch (alist->interpolation()) {
			case AutomationList::Discrete:
...
timecnt_t
AutomationLineBase::model_to_view_coord_x (timepos_t const & when) const
{
...
void
AutomationLineBase::interpolation_changed (AutomationList::InterpolationStyle style)
{
	if (line_points.size() > 1) {
		reset ();
		line->set_steps(line_points, is_stepped());
...
void
AutomationLineBase::add_visible_control_point (uint32_t view_index, uint32_t pi, double tx, double ty,
...
void
AutomationLineBase::dump (std::ostream& ostr) const
{
	for (auto const & cp : control_points) {
...
void
AutomationLineBase::connect_to_list ()
{
	_list_connections.drop_connections ();

	alist->StateChanged.connect (_list_connections, invalidator (*this), boost::bind (&AutomationLineBase::list_changed, this), gui_context());

	alist->InterpolationChanged.connect (
		_list_connections, invalidator (*this), boost::bind (&AutomationLineBase::interpolation_changed, this, _1), gui_context());
}

MementoCommandBinder<AutomationList>*
AutomationLineBase::memento_command_binder ()
{
 ...
 */
void
AutomationLineBase::set_maximum_time (Temporal::timepos_t const & t)
{
	if (_maximum_time == t) {
		return;
	}
...
AutomationLineBase::get_point_x_range () const
{
...
timepos_t
AutomationLineBase::session_position (timepos_t const & when) const
{
	return when + get_origin();
}

void
AutomationLineBase::set_offset (timepos_t const & off)
{
	_offset = off;
	reset ();
}
