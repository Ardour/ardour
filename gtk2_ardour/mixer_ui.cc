...
#undef BUSNAME
	...
	if (group) {
		...
		const std::list<std::shared_ptr<RouteGroup>>& rgs (_session->route_groups ());
		for (std::list<std::shared_ptr<RouteGroup>>::const_iterator i = rgs.begin (); i != rgs.end (); ++i) {
			if ((*i)->name () == pi->name ()) {
				rg = *i;
				break;
			}
...
Mixer_UI::route_group_row_deleted (Gtk::TreeModel::Path const &)
{
	...
	list<std::shared_ptr<RouteGroup>> new_list;
