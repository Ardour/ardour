...
RTAWindow* RTAWindow::_instance = 0;

RTAWindow*
RTAWindow::instance ()
{
	if (!_instance) {
		_instance = new RTAWindow;
		/* just in case the instance is not created by the window manager */
		_instance->set_session (ARDOUR_UI::instance()->the_session ());
	}
...
RTAWindow::RTAWindow ()
	: ArdourWindow (_("Real Time Analyzer"))
	...
	, _dragging_dB (DragNone)
{
	...
	_speed_dropdown.set_text (_speed_strings[(int)_speed]);
	...
	_warp_dropdown.set_text (_warp_strings[(int)_warp]);
...
RTAWindow::set_session (ARDOUR::Session* s)
{
	...
	_update_connection = Timers::super_rapid_connect (sigc::mem_fun (*this, &RTAWindow::fast_update));
}
...
RTAWindow::update_title ()
{
	if (_session) {
		...
		Gtkmm2ext::WindowTitle title (S_("Window|RTA"));
		title += Glib::get_application_name ();
		set_title (title.get_string ());
	}
}
...
{
	set_active (_visible && !_pause.get_active ());
}

void
RTAWindow::route_removed (std::weak_ptr<Route> wr)
{
...
void
RTAWindow::attach (std::shared_ptr<ARDOUR::Route> route)
{
	for (auto const& r : _rta) {
		if (r.route == route) {
			return;
	...
	_rta.back ().route->main_outs ()->set_analysis_active (_visible && !_pause.get_active ());

	route->DropReferences.connect (*this, invalidator (*this), std::bind (&RTAWindow::route_removed, this, std::weak_ptr<Route>(route)), gui_context());
}

void
RTAWindow::remove (std::shared_ptr<ARDOUR::Route> route)
{
	_rta.remove_if ([route] (RTAWindow::RTA const& r) { return r.route == route; });
}

bool
RTAWindow::attached (std::shared_ptr<ARDOUR::Route> route) const
{
	return std::find_if (_rta.begin (), _rta.end (), [route] (RTAWindow::RTA const& r) { return r.route == route; }) != _rta.end ();
}
...
RTAWindow::run_rta ()
{
	...
	for (auto& r : _rta) {
		...
		while (r.ringbuffer->read_space () >= stepsize) {
			r.ringbuffer->read (r.analyzer.ipdata () + r.offset, stepsize);
...
RTAWindow::darea_motion_notify_event (GdkEventMotion* ev)
{
	...
	if (_dragging_dB != DragNone) {
		...
		if (_dragging_dB == DragUpper) {
			new_dB = rintf (std::min (_dB_min + _dB_range, std::max (new_dB, _min_dB + 24.f)));
			changed = _max_dB != new_dB;
			_max_dB = new_dB;
		} else if (_dragging_dB == DragLower) {
			new_dB = rintf (std::max (_dB_min, std::min (new_dB, _max_dB - 24.f)));
			changed = _min_dB != new_dB;
			_min_dB = new_dB;
		} else {
			float min_dB = rintf (std::max (_dB_min, std::min (new_dB, _max_dB - 24.f)));
			float dbd = (min_dB - _min_dB);
			float max_dB = rintf (std::min (_dB_min + _dB_range, std::max (_max_dB + dbd, _min_dB + 24.f)));
	...
	if (ev->x > _margin && ev->x < width - _margin) {
		float freq = freq_at_x (ev->x - _margin, width - 2 * _margin);
...
#if 0 // TODO
		if (!_rta.empty()) {
			// find closest bin for given freq (or px)
			float db = _rta.front().analyzer.power_at_bin (b);
			ss << " " << std::setw (6) << std::setprecision (1) << std::showpos << dB;
			ss << std::setw (0) << "dB";
		}
		// consider rendering cursor lines for pointer
#endif
...
RTAWindow::darea_expose_event (GdkEventExpose* ev)
{
	...
	if (!_grid) {
		_grid = Cairo::ImageSurface::create (Cairo::FORMAT_ARGB32, width, height);
	...
	if (_xpos.empty () && !_rta.empty ()) {
		auto const& r      = _rta.front ();
		const int   n_bins = r.analyzer.fftlen ();
		for (int i = 0; i <= n_bins; ++i) {
			float f = r.analyzer.freq_at_bin (i);
			if (f < 15 || f > 22000) {
				_xpos[i] = -1;
			} else {
	...
	for (auto const& r : _rta) {
		...
		for (int i = 0; i <= n_bins; ++i) {
			...
			float y = y1 - hh * (r.analyzer.power_at_bin (i) - min_dB) / (max_dB - min_dB);
...
RTAWindow::RTA::RTA (std::shared_ptr<ARDOUR::Route> r)
	: route (r)
...
{
	ringbuffer = RTARingBufferPtr (new RTARingBuffer (rate), std::bind (&rt_safe_delete<RTARingBuffer>, &r->session (), _1));
	route->main_outs ()->set_analysis_ringbuffer (ringbuffer);
...
RTAWindow::RTA::~RTA ()
{
	RTARingBufferPtr unset;
	route->main_outs ()->set_analysis_ringbuffer (unset);
}
