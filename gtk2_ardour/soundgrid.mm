...
static bool
soundgrid_driver_init (uint32_t max_phys_inputs, uint32_t max_phys_outputs)
{
        if (ARDOUR::SoundGrid::instance().configure_driver (max_phys_inputs, max_phys_outputs) == 0) {
                ARDOUR::SoundGrid::instance().map_inputs_as_jack_ports (max_phys_inputs);
        }
...
int
soundgrid_init (uint32_t max_track_inputs, uint32_t max_track_ouputs,
                uint32_t max_phys_inputs, uint32_t max_phys_outputs)
{
        ...
        */
        ...
        if (ARDOUR::SoundGrid::instance().initialize ([sg_window contentView], max_track_inputs, max_track_ouputs,
        ...
        Glib::signal_timeout().connect (sigc::bind (sigc::ptr_fun (soundgrid_driver_init), max_phys_inputs, max_phys_outputs), 2000);
