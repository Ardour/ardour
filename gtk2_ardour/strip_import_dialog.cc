...
StripImportDialog::setup_file_page ()
{
	...
	if (templates.size () > 0) {
		...
		_template_treeview.get_selection ()->signal_changed ().connect (sigc::bind (sigc::mem_fun (*this, &StripImportDialog::treeview_selection_changed), &_template_treeview));
	...
	local_path.add_subdirectory_to_paths ("mix_presets"); // XXX
	...
	if (templates.size () > 0) {
		...
		_local_pset_treeview.get_selection ()->signal_changed ().connect (sigc::bind (sigc::mem_fun (*this, &StripImportDialog::treeview_selection_changed), &_local_pset_treeview));
	...
	global_path.add_subdirectory_to_paths ("mix_presets"); // XXX
	...
	if (templates.size () > 0) {
		...
		_global_pset_treeview.get_selection ()->signal_changed ().connect (sigc::bind (sigc::mem_fun (*this, &StripImportDialog::treeview_selection_changed), &_global_pset_treeview));
...
StripImportDialog::page_changed (GtkNotebookPage*, guint page)
{
	switch (page) {
		case 0:
			...
			break;
		default:
			treeview_selection_changed (_notebook_content.at (page));
			break;
	}
}
...
static string
template_dir_to_file (string const& dir)
{
	string path = Glib::build_filename (dir, Glib::path_get_basename(dir) + template_suffix);
	if (!Glib::file_test (path, Glib::FILE_TEST_IS_REGULAR)) {
		path = Glib::build_filename (dir, Glib::path_get_basename(dir) + ".axml"); // XXX
	}
	return path;
}

void
StripImportDialog::treeview_selection_changed (Gtk::TreeView* treeview)
{
	Gtk::TreeModel::const_iterator selection = treeview->get_selection ()->get_selected ();
	if (selection) {
		parse_track_state (template_dir_to_file ((*selection)[_columns.path]));
	} else {
		parse_track_state ("");
	}
}
