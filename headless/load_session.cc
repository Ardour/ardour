...
static bool enable_plugins = false;
static int plugin_timeout_ms = 30000;
static bool strict_plugin_loading = false;
static string vst_path;
static string plugin_blacklist_file;
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, optstring, longopts, (int *)0)) != EOF)
	{
		...
		case 'T':
			plugin_timeout_ms = atoi(optarg);
			if (plugin_timeout_ms <= 0)
			{
				cerr << "Invalid plugin timeout value: " << optarg << endl;
				exit(EXIT_FAILURE);
			}
			break;
	...
	// Enhanced plugin manager initialization for headless mode
	if (enable_plugins)
	{
