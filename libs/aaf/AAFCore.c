...
static cfbNode *getStrongRefEntryNode(AAF_Data *aafd, aafObject *Parent,
                                      const char *refName, uint32_t index);
...
aafUID_t *aaf_get_InterpolationIdentificationByWeakRef(
    AAF_Data *aafd, aafWeakRef_t *InterpolationDefWeakRef) {
  aafObject *InterpolationDefinition = aaf_get_ObjectByWeakRef(
      aafd->InterpolationDefinition, InterpolationDefWeakRef);
...
aafUID_t *
aaf_get_OperationIdentificationByWeakRef(AAF_Data *aafd,
...
aafUID_t *
aaf_get_ContainerIdentificationByWeakRef(AAF_Data *aafd,
                                         aafWeakRef_t *ContainerDefWeakRef) {
  aafObject *ContainerDefinition =
      aaf_get_ObjectByWeakRef(aafd->ContainerDefinition, ContainerDefWeakRef);

  if (!ContainerDefinition) {
    warning(
        "Could not retrieve WeakRef from Dictionary::ContainerDefinitions.");
    return NULL;
  }
...
aafUID_t *aaf_get_DataIdentificationByWeakRef(AAF_Data *aafd,
                                              aafWeakRef_t *DataDefWeakRef) {
  aafObject *DataDefinition =
      aaf_get_ObjectByWeakRef(aafd->DataDefinition, DataDefWeakRef);
...
aafObject *aaf_get_ObjectAncestor(aafObject *Obj, const aafUID_t *ClassID) {
  ...
  while (Obj != NULL &&
         !aafUIDCmp(Obj->Class->ID, &AAFClassID_ContentStorage)) {
...
aafObject *aaf_get_EssenceDataByMobID(AAF_Data *aafd, aafMobID_t *MobID) {
  ...
  for (EssenceData = aafd->EssenceData; EssenceData != NULL;
...
aafUID_t *aaf_get_ParamDefIDByName(AAF_Data *aafd, const char *name) {
  ...
  aafObject *ParameterDefinitions = aaf_get_propertyValue(
  ...
  AAF_foreach_ObjectInSet(&ParameterDefinition, ParameterDefinitions, NULL) {
    ...
    char *paramName = aaf_get_propertyValue(
        ParameterDefinition, PID_DefinitionObject_Name, &AAFTypeID_String);
    ...
    if (strcmp(paramName, name) == 0) {
      ParamDefIdent = aaf_get_propertyValue(ParameterDefinition,
...
void *aaf_get_TaggedValueByName(AAF_Data *aafd, aafObject *TaggedValueVector,
                                const char *name, const aafUID_t *type) {
  ...
  AAF_foreach_ObjectInSet(&TaggedValue, TaggedValueVector, NULL) {
    ...
    char *taggedName = aaf_get_propertyValue(TaggedValue, PID_TaggedValue_Name,
                                             &AAFTypeID_String);
    aafIndirect_t *taggedIndirect = aaf_get_propertyValue(
        TaggedValue, PID_TaggedValue_Value, &AAFTypeID_Indirect);

    if (strcmp(taggedName, name) == 0) {

      if (aafUIDCmp(&taggedIndirect->TypeDef, type)) {

        debug("Found TaggedValue \"%s\" of type %s", taggedName,
      ...
      debug("Got TaggedValue \"%s\" but of type %s instead of %s", taggedName,
            aaft_TypeIDToText(&taggedIndirect->TypeDef),
            aaft_TypeIDToText(type));
    }
    // LOG_BUFFER_WRITE( log, "     %sTagged > Name: %s%s%s%*s      Value:
    // %s(%s)%s %s%s%s", 	ANSI_COLOR_RESET(log), 	ANSI_COLOR_DARKGREY(log), 	(name)
    // ? name : "<unknown>", 	ANSI_COLOR_RESET(log), 	(name) ?
    // (size_t)(24-(int)strlen(name)) : (size_t)(24-strlen("<unknown>")), " ",
...
aafUID_t *aaf_get_OperationDefIDByName(AAF_Data *aafd, const char *OpDefName) {
  ...
  while (_aaf_foreach_ObjectInSet(&OperationDefinition, OperationDefinitions,
                                  NULL)) {

    aafUID_t *OpDefIdent = aaf_get_propertyValue(
        OperationDefinition, PID_DefinitionObject_Identification,
        &AAFTypeID_AUID);

    char *name = aaf_get_propertyValue(
        OperationDefinition, PID_DefinitionObject_Name, &AAFTypeID_String);
...
aafProperty *aaf_get_property(aafObject *Obj, aafPID_t pid) {
  ...
  if (!Prop) {
    ...
    if (!PDef) {
      warning("Could not retrieve 0x%04x (%s) of Class %s", pid,
...
void *aaf_get_propertyValue(aafObject *Obj, aafPID_t pid,
  ...
  if (aafUIDCmp(typeID, &AAFTypeID_Indirect)) {
    ...
    if (aafUIDCmp(&Indirect->TypeDef, &AAFTypeID_String) &&
      ...
      error("Object %s Indirect::string property 0x%04x (%s) does not end with "
            "NULL",
            aaft_ClassIDToText(aafd, Obj->Class->ID), pid,
            aaft_PIDToText(aafd, pid));
      return NULL;
...
static int parse_Identification(AAF_Data *aafd) {
  ...
  char *Company = aaf_get_propertyValue(Identif, PID_Identification_CompanyName,
                                        &AAFTypeID_String);
  ...
  char *ProductName = aaf_get_propertyValue(
  ...
  char *ProductVersionString = aaf_get_propertyValue(
  ...
  char *Platform = aaf_get_propertyValue(Identif, PID_Identification_Platform,
                                         &AAFTypeID_String);
...
static int retrieveObjectTree(AAF_Data *aafd) {
  ...
  if (!propStream) {
    error("Could not retrieve properties for %s.",
...
static aafClass *retrieveMetaDictionaryClass(AAF_Data *aafd,
  ...
  if (!ClassDefs) { /* opt */
    debug("Could not retrieve PID_MetaDictionary_ClassDefinitions property "
          "from MetaDic.");
    return NULL;
  }
  ...
  if (!ClassID) { /* req */
    error("Could not retrieve PID_MetaDefinition_Identification property from "
          "ClassDef.");
    return NULL;
  }
  ...
  if (!parent) {
    error("Could not retrieve PID_ClassDefinition_ParentClass property from "
          "ClassDef.");
    return NULL;
  }
  ...
  if (!Parent) {
    error("Could not retrieve object by weakRef "
          "(PID_ClassDefinition_ParentClass)");
    return NULL;
  }
  ...
  if (Parent != ClassDef) {
     ...
     */
    error("Parent's Class equals Child's : %s.",
  ...
  if (!Class) {
    ...
    if (!Class->name) {
      debug("Could not retrieve PID_MetaDefinition_Name property from ClassDef "
            "(%s)",
            aaft_ClassIDToText(aafd, ClassID));
    }
  } else {
    ...
    if (!Class->name) {
      ...
      if (!Class->name) {
        debug("Could not retrieve PID_MetaDefinition_Name property from "
              "ClassDef (%s)",
              aaft_ClassIDToText(aafd, ClassID));
      }
    }
  ...
  if (!Props) { /* opt */
    debug("Could not retrieve PID_ClassDefinition_Properties property from "
          "ClassDef (%s)",
          aaft_ClassIDToText(aafd, ClassID));
  }
  ...
  AAF_foreach_ObjectInSet(&Prop, Props, NULL) {
    ...
    if (!PDef->name) {
      warning("Could not retrieve PID_MetaDefinition_Name property from "
              "PropertyDefinition.");
    }
    ...
    // char *typeName = aaf_get_propertyValue( TypeDef, PID_MetaDefinition_Name,
    // &AAFTypeID_String );
    //
...
static int retrieveStrongReference(AAF_Data *aafd, aafProperty *Prop,
                                   aafObject *Parent) {
  /*
   * Initial property value is a unicode string holding the name of a child
   * node. This child node being the object referenced, we store that object
   * directly as the property value, instead of the initial child node name.
   */
...
static int retrieveObjectProperties(AAF_Data *aafd, aafObject *Obj) {
  ...
  foreachPropertyEntry(propStream, Header, Prop, value, valueOffset, i) {
    ...
    if (!PDef) {
      warning("Unknown property 0x%04x (%s) of object %s", Prop._pid,
...
static cfbNode *getStrongRefIndexNode(AAF_Data *aafd, aafObject *Parent,
  ...
  if (!Node) {
    error(
        "Could not retrieve Reference Set/Vector Index Node @ \"%s/%s index\"",
...
static cfbNode *getStrongRefEntryNode(AAF_Data *aafd, aafObject *Parent,
  ...
  if (!Node) {
    error(
        "Could not retrieve Reference Set/vector Entry Node @ \"%s/%s index\"",
        aaf_get_ObjectPath(Parent), refName);
    return NULL;
  }
  ...
  /*
     ...
     + sizeof(aafPropertyIndexEntry_t);
          ...
          if ( prop_sz != stream_sz )
                  warning( L"Stream length (%lu Bytes) does not match property
...
getStrongRefSetList(AAF_Data *aafd, cfbNode *Node, aafObject *Parent) {
  ...
  if (!stream) {
    ...
    error(
        "Could not retrieve StrongReferenceSet Index Stream @ \"%s/%s index\"",
        aaf_get_ObjectPath(Parent), refName);
...
static aafByte_t *getStrongRefVectorList(AAF_Data *aafd, cfbNode *Node,
  ...
  if (!stream) {
    ...
    error(
        "Could not retrieve StrongReferenceVector Index Stream \"%s/%s index\"",
        aaf_get_ObjectPath(Parent), refName);
