...
void aaf_dump_Header(AAF_Data *aafd, const char *padding) {
  ...
  LOG_BUFFER_WRITE(log, "%sByteOrder            : %s%s (0x%04x)%s\n", padding,
                   ...
                   aafd->Header.ByteOrder, ANSI_COLOR_RESET(log));
  ...
  LOG_BUFFER_WRITE(log, "%sObjectModel Version  : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), aafd->Header.ObjectModelVersion,
                   ANSI_COLOR_RESET(log));
  ...
  log->log_callback(log, (void *)aafd, LOG_SRC_ID_DUMP, 0, "", "", 0, log->_msg,
                    log->user);
}
...
void aaf_dump_ObjectProperty(AAF_Data *aafd, aafProperty *Prop,
  ...
  if (Prop->def->meta) {
    LOG_BUFFER_WRITE(log, "%s%s[%s0x%04x%s] %s (%s)\n", padding,
                     ANSI_COLOR_RESET(log), ANSI_COLOR_MAGENTA(log), Prop->pid,
                     ANSI_COLOR_RESET(log), aaft_PIDToText(aafd, Prop->pid),
                     aaft_StoredFormToText(Prop->sf));
  } else {
    LOG_BUFFER_WRITE(log, "%s%s[%s0x%04x%s] %s (%s)\n", padding,
                     ANSI_COLOR_RESET(log), ANSI_COLOR_DARKGREY(log), Prop->pid,
                     ANSI_COLOR_RESET(log), aaft_PIDToText(aafd, Prop->pid),
                     aaft_StoredFormToText(Prop->sf));
  }

  int rc =
      laaf_util_dump_hex(Prop->val, Prop->len, &aafd->log->_msg,
                         &aafd->log->_msg_size, aafd->log->_msg_pos, padding);
...
void aaf_dump_TaggedValueSet(AAF_Data *aafd, aafObject *ObjCollection,
  ...
  AAF_foreach_ObjectInSet(&Obj, ObjCollection, NULL) {
    ...
    aafIndirect_t *indirect =
        aaf_get_propertyValue(Obj, PID_TaggedValue_Value, &AAFTypeID_Indirect);

    LOG_BUFFER_WRITE(
        log, "%s%sTagged > Name: %s%s%s%*s      Value: %s(%s)%s %s%s%s%s%s\n",
        padding, ANSI_COLOR_RESET(log), ANSI_COLOR_DARKGREY(log),
        (name) ? name : "<unknown>", ANSI_COLOR_RESET(log),
        (name) ? (size_t)(34 - (int)strlen(name))
               : (size_t)(34 - strlen("<unknown>")),
        " ", ANSI_COLOR_DARKGREY(log), aaft_TypeIDToText(&indirect->TypeDef),
    ...
    log->log_callback(log, (void *)aafd, LOG_SRC_ID_DUMP, 0, "", "", 0,
                      log->_msg, log->user);
...
void aaf_dump_rawProperties(AAF_Data *aafd, aafByte_t *propStream,
  ...
  if (propStream == NULL) {
    LOG_BUFFER_WRITE(log,
                     "%s## "
                     "Property_Header__________________________________________"
                     "__________\n\n"
  ...
  LOG_BUFFER_WRITE(
      ...
      "%s======================================================================"
      "\n\n",
      padding, padding, ANSI_COLOR_DARKGREY(log), Header._byteOrder,
      ANSI_COLOR_RESET(log), padding, ANSI_COLOR_DARKGREY(log),
      Header._formatVersion, ANSI_COLOR_RESET(log), padding,
      ANSI_COLOR_DARKGREY(log), Header._entryCount, ANSI_COLOR_RESET(log),
      padding);
  ...
  foreachPropertyEntry(propStream, Header, Prop, value, valueOffset, i) {
    LOG_BUFFER_WRITE(
        ...
        padding, i, padding, ANSI_COLOR_DARKGREY(log), Prop._pid,
        aaft_PIDToText(aafd, Prop._pid), ANSI_COLOR_RESET(log), padding,
        ANSI_COLOR_DARKGREY(log), aaft_StoredFormToText(Prop._storedForm),
        ANSI_COLOR_RESET(log), padding, ANSI_COLOR_DARKGREY(log), Prop._length,
        ANSI_COLOR_RESET(log));

    int rc =
        laaf_util_dump_hex(value, Prop._length, &aafd->log->_msg,
                           &aafd->log->_msg_size, aafd->log->_msg_pos, padding);
    ...
    foreachPropertyDefinition(PDef, Class->Properties) {
      if (Class->meta) {
        LOG_BUFFER_WRITE(log, "%s%s%s::%s (0x%04x)%s\n", padding,
                         ANSI_COLOR_MAGENTA(log), Class->name, PDef->name,
                         PDef->pid, ANSI_COLOR_RESET(log));

        print++;
      } else if (PDef->meta) {
        LOG_BUFFER_WRITE(log, "%s%s::%s%s (0x%04x)%s\n", padding,
                         aaft_ClassIDToText(aafd, Class->ID),
                         ANSI_COLOR_MAGENTA(log), PDef->name, PDef->pid,
                         ANSI_COLOR_RESET(log));
...
void aaf_dump_Classes(AAF_Data *aafd, const char *padding) {
  ...
  foreachClass(ConcreteClass, aafd->Classes) {
    foreachClassInheritance(Class, ConcreteClass) {
      LOG_BUFFER_WRITE(log, "%s%s%s%s", padding,
