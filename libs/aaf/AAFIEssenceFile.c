...
static int set_audioEssenceWithRIFF(AAF_Iface *aafi, const char *filename,
...
int aafi_build_unique_audio_essence_name(
  ...
  if (aafi->ctx.options.mobid_essence_filename) {
    ...
    int rc = laaf_util_snprintf_realloc(
        &audioEssenceFile->unique_name, 0, 0,
        "%08x-%04x-%04x-%02x%02x%02x%02x%02x%02x%02x%02x", uuid->Data1,
        uuid->Data2, uuid->Data3, uuid->Data4[0], uuid->Data4[1],
    ...
    audioEssenceFile->unique_name = laaf_util_c99strdup(
        (audioEssenceFile->name) ? audioEssenceFile->name : "unknown");

    if (!audioEssenceFile->unique_name) {
      error("Could not duplicate essence name : %s",
            (audioEssenceFile->name) ? audioEssenceFile->name : "unknown");
      return -1;
    }
  }
  ...
  AAFI_foreachAudioEssenceFile(aafi, ae) {
    ...
    if (ae != audioEssenceFile && ae->unique_name != NULL &&
        strcmp(ae->unique_name, audioEssenceFile->unique_name) == 0) {

      if (laaf_util_snprintf_realloc(
              &audioEssenceFile->unique_name, &unique_size, 0, "%s_%i",
              (audioEssenceFile->name) ? audioEssenceFile->name : "unknown",
              ++index) < 0) {
...
char *aafi_locate_external_essence_file(AAF_Iface *aafi,
  ...
  if (search_location) {
    ...
    local_filepath =
        laaf_util_build_path("/", search_location, essenceFileName, NULL);
    ...
    local_filepath =
        laaf_util_build_path("/", search_location, relativeEssencePath, NULL);
...
found:
  if (foundpath) {
    /*
     * When runing through wine, computing absolute path adds a Z:/ drive
     * letter. This causes issue when trying to make relative essence path from
     * the AAF file path, since it also went through laaf_util_absolute_path().
...
int aafi_extractAudioEssenceFile(AAF_Iface *aafi,
                                 ...
                                 const char *outpath, uint64_t sampleOffset,
  ...
  uint64_t pcmByteOffset = sampleOffset * audioEssenceFile->channels *
                           (audioEssenceFile->samplesize / 8);
  uint64_t pcmByteLength = sampleLength * audioEssenceFile->channels *
                           (audioEssenceFile->samplesize / 8);
  ...
  debug(" -    ReqSampleOffset: %" PRIu64 " samples (%" PRIu64 " bytes)",
        sampleOffset, pcmByteOffset);
  debug(" -    ReqSampleLength: %" PRIu64 " samples (%" PRIu64 " bytes)",
        sampleLength, pcmByteLength);
  ...
  if (pcmByteOffset || pcmByteLength || extractFormat != AAFI_EXTRACT_DEFAULT) {
  ...
  if ((datasz - audioEssenceFile->pcm_audio_start_offset) <
      (pcmByteLength + sourceFileOffset)) {
    error("Requested audio range (%" PRIi64
          " bytes) is bigger than source audio size (%" PRIu64 " bytes)",
          (pcmByteLength + sourceFileOffset),
  ...
  if (write_header || audioEssenceFile->type == AAFI_ESSENCE_TYPE_WAVE ||
  ...
  if (fileext) {
    if (laaf_util_snprintf_realloc(&filename, NULL, 0, "%s.%s", name, fileext) <
        0) {
  ...
  filepath = laaf_util_build_path(DIR_SEP_STR, outpath,
  ...
  if (write_header || audioEssenceFile->type == AAFI_ESSENCE_TYPE_PCM) {
    ...
    tmp = snprintf(wavBext.originator,
                   sizeof(((struct wavBextChunk *)0)->originator), "%s %s",
                   aafi->aafd->Identification.ProductName,
                   (mediaComposer_AAF(aafi))
                       ? ""
                       : aafi->aafd->Identification.ProductVersionString);

    assert(tmp > 0 &&
           (size_t)tmp < sizeof(((struct wavBextChunk *)0)->originator));

    // if ( tmp < 0 || (size_t)tmp >= sizeof(((struct wavBextChunk
    // *)0)->originator) ) { 	fprintf( stderr, "snprintf() error" ); 	goto err;
    // }

    tmp = snprintf(wavBext.originator_reference,
                   sizeof(((struct wavBextChunk *)0)->originator_reference),
                   "libAAF %s", LIBAAF_VERSION);
    ...
    // if ( tmp < 0 || (size_t)tmp >= sizeof(((struct wavBextChunk
    // *)0)->originator_reference) ) { 	fprintf( stderr, "snprintf() error" );
    // 	goto err;
    // }

    tmp = snprintf(wavBext.description,
                   sizeof(((struct wavBextChunk *)0)->description),
                   "%s\n%s.aaf", audioEssenceFile->name, aafi->compositionName);

    assert(tmp > 0 &&
           (size_t)tmp < sizeof(((struct wavBextChunk *)0)->description));

    // if ( tmp < 0 || (size_t)tmp >= sizeof(((struct wavBextChunk
    // *)0)->description) ) { 	fprintf( stderr, "snprintf() error" ); 	goto err;
    // }

    memcpy(wavBext.origination_date, audioEssenceFile->originationDate,
           sizeof(((struct wavBextChunk *)0)->origination_date));
    memcpy(wavBext.origination_time, audioEssenceFile->originationTime,
           sizeof(((struct wavBextChunk *)0)->origination_time));

    wavBext.time_reference =
        aafi_convertUnitUint64(audioEssenceFile->sourceMobSlotOrigin,
                               audioEssenceFile->sourceMobSlotEditRate,
                               audioEssenceFile->samplerateRational);

    if (laaf_riff_writeWavFileHeader(
            fp, &wavFmt, (extractFormat != AAFI_EXTRACT_WAV) ? &wavBext : NULL,
            (uint32_t)datasz, aafi->log) < 0) {
  ...
  if (write_header && audioEssenceFile->type == AAFI_ESSENCE_TYPE_AIFC &&
      audioEssenceFile->samplesize > 8) {
    ...
    writtenBytes =
        fwrite(data + sourceFileOffset, sizeof(unsigned char), datasz, fp);
  }
  ...
  if (!extracting_clip) {
    /*
     * Set audioEssenceFile->usable_file_path only if we axtract essence, not if
     * we extract clip (subset of an essence), as a single essence can have
     * multiple clips using it. Otherwise, we would reset
     * audioEssenceFile->usable_file_path as many times as there are clips using
     * the same essence.
     */
...
int aafi_extractAudioClip(AAF_Iface *aafi, aafiAudioClip *audioClip,
  ...
  for (aafiAudioEssencePointer *audioEssencePtr = audioClip->essencePointerList;
       audioEssencePtr; audioEssencePtr = audioEssencePtr->next) {

    aafiAudioEssenceFile *audioEssenceFile =
        audioClip->essencePointerList->essenceFile;

    uint64_t sampleOffset = aafi_convertUnitUint64(
        audioClip->essence_offset, audioClip->track->edit_rate,
        audioEssenceFile->samplerateRational);
    uint64_t sampleLength =
        aafi_convertUnitUint64(audioClip->len, audioClip->track->edit_rate,
                               audioEssenceFile->samplerateRational);
    ...
    laaf_util_snprintf_realloc(
        &name, NULL, 0, "%i_%i_%s", audioClip->track->number,
        aafi_get_clipIndex(audioClip),
        audioClip->essencePointerList->essenceFile->unique_name);

    if ((rc += aafi_extractAudioEssenceFile(
             aafi, audioEssenceFile, extractFormat, outpath, sampleOffset,
             sampleLength, name, &usable_file_path)) == 0) {
      success("Audio clip file extracted to %s\"%s\"%s",
              ANSI_COLOR_DARKGREY(aafi->log), usable_file_path,
              ANSI_COLOR_RESET(aafi->log));
    } else {
      error("Audio clip file extraction failed : %s\"%s\"%s",
            ANSI_COLOR_DARKGREY(aafi->log), name, ANSI_COLOR_RESET(aafi->log));
    }
...
static int set_audioEssenceWithRIFF(AAF_Iface *aafi, const char *filename,
  ...
  if (RIFFAudioFile->sampleCount >= INT64_MAX) {
    error("%s : summary sample count is bigger than INT64_MAX (%" PRIu64 ")",
          audioEssenceFile->usable_file_path, RIFFAudioFile->sampleCount);
    return -1;
  }

  if (RIFFAudioFile->sampleRate >= INT_MAX) {
    error("%s : summary sample rate is bigger than INT_MAX (%li)",
          audioEssenceFile->usable_file_path, RIFFAudioFile->sampleRate);
    return -1;
  }

  if (audioEssenceFile->channels > 0 &&
      audioEssenceFile->channels != RIFFAudioFile->channels) {
    warning("%s : summary channel count (%i) mismatch %s (%i)", filename,
            audioEssenceFile->channels,
            ((isExternalFile) ? "located file" : "previously retrieved data"),
            RIFFAudioFile->channels);
  }
  // else {
  // 	/* In Davinci Resolve embedded multichannel WAV, summary channel is
  // always 1 */ 	audioEssenceFile->channels = RIFFAudioFile->channels;
  // }

  if (audioEssenceFile->samplerate > 0 &&
      audioEssenceFile->samplerate != RIFFAudioFile->sampleRate) {
    warning("%s : summary samplerate (%i) mismatch %s (%i)", filename,
            audioEssenceFile->samplerate,
            ((isExternalFile) ? "located file" : "previously retrieved data"),
            RIFFAudioFile->sampleRate);
  }

  if (audioEssenceFile->samplesize > 0 &&
      audioEssenceFile->samplesize != RIFFAudioFile->sampleSize) {
    warning("%s : summary samplesize (%i) mismatch %s (%i)", filename,
            audioEssenceFile->samplesize,
            ((isExternalFile) ? "located file" : "previously retrieved data"),
            RIFFAudioFile->sampleSize);
  }

  if (audioEssenceFile->length > 0 &&
      (uint64_t)audioEssenceFile->length != RIFFAudioFile->sampleCount) {
    warning("%s : summary samplecount (%" PRIi64 ") mismatch %s (%" PRIi64 ")",
            filename, audioEssenceFile->length,
            ((isExternalFile) ? "located file" : "previously retrieved data"),
  ...
  audioEssenceFile->pcm_audio_start_offset =
      (uint64_t)RIFFAudioFile->pcm_audio_start_offset;
  audioEssenceFile->samplerateRational->numerator =
      (int32_t)audioEssenceFile->samplerate;
...
int aafi_parse_audio_essence(AAF_Iface *aafi,
  ...
  if (audioEssenceFile->summary) {
    ...
    rc = laaf_riff_parseAudioFile(
        &RIFFAudioFile, RIFF_PARSE_AAF_SUMMARY,
        &embeddedAudioDataReaderCallback, audioEssenceFile->summary->val,
        &audioEssenceFile->summary->len, aafi, aafi->log);

    if (rc < 0) {

      if (!audioEssenceFile->is_embedded &&
          !audioEssenceFile->usable_file_path) {
        warning("Could not parse essence summary of \"%s\".",
      ...
      warning("Could not parse essence summary of \"%s\". %s",
              ...
              : (audioEssenceFile->usable_file_path)
                  ? "Trying external essence file."
                  : " WTF ???");
    } else {

      if (set_audioEssenceWithRIFF(aafi, "AAF Summary", audioEssenceFile,
                                   &RIFFAudioFile, 0) < 0) {
        goto err;
      }

      if (!RIFFAudioFile.channels || !RIFFAudioFile.sampleRate ||
          !RIFFAudioFile.sampleSize || !RIFFAudioFile.sampleCount) {
        /*
        ...
        if (!audioEssenceFile->is_embedded &&
            !audioEssenceFile->usable_file_path) {
          warning("Summary of \"%s\" is missing some data.",
                  audioEssenceFile->name);
          goto err;
        }
    ...
    warning("Embedded audio essence \"%s\" has no summary. Trying essence data "
            "stream.",
            audioEssenceFile->name);
  } else if (audioEssenceFile->usable_file_path) {
    warning(
        "External audio essence \"%s\" has no summary. Trying external file.",
        audioEssenceFile->name);
  }

  if (audioEssenceFile->is_embedded) {

    cfb_getStream(aafi->aafd->cfbd, audioEssenceFile->node, &dataStream,
                  &dataStreamSize);
    ...
    rc = laaf_riff_parseAudioFile(&RIFFAudioFile, RIFF_PARSE_AAF_SUMMARY,
                                  &embeddedAudioDataReaderCallback, dataStream,
                                  &dataStreamSize, aafi, aafi->log);

    if (rc < 0) {
      warning("Could not parse embedded essence stream of \"%s\".",
              audioEssenceFile->name);
      goto err;
    }

    if (set_audioEssenceWithRIFF(aafi, "AAF Embedded stream", audioEssenceFile,
                                 &RIFFAudioFile, 0) < 0) {
      goto err;
    }
  ...
  if (laaf_util_is_fileext(audioEssenceFile->usable_file_path, "wav") ||
    ...
    rc = laaf_riff_parseAudioFile(
        &RIFFAudioFile, 0, &externalAudioDataReaderCallback, fp,
        audioEssenceFile->usable_file_path, aafi, aafi->log);
    ...
    if (set_audioEssenceWithRIFF(aafi, audioEssenceFile->usable_file_path,
                                 audioEssenceFile, &RIFFAudioFile, 1) < 0) {
      goto err;
    }
  } else {
    /*
     ...
     *
│ 04317│├──◻ AAFClassID_TimelineMobSlot [slot:6 track:4] (DataDef :
AAFDataDef_Sound) : Audio 4 - Layered Audio Editing │ 01943││    └──◻
AAFClassID_Sequence │ 02894││         └──◻ AAFClassID_SourceClip │ 02899││ └──◻
AAFClassID_MasterMob (UsageCode: n/a) : speech-sample │ 04405││ └──◻
AAFClassID_TimelineMobSlot [slot:1 track:1] (DataDef : AAFDataDef_Sound) │
03104││                        └──◻ AAFClassID_SourceClip │ 04140││ └──◻
AAFClassID_SourceMob (UsageCode: n/a) : speech-sample │ 01287││ └──◻
AAFClassID_PCMDescriptor │ 01477││                                       └──◻
...
static size_t externalAudioDataReaderCallback(unsigned char *buf, size_t offset,
  ...
  if (_fseeki64(fp, (__int64)offset, SEEK_SET) < 0) {
    error("Could not seek to %" PRIu64 " in file '%s' : %s", offset, filename,
          strerror(errno));
    return RIFF_READER_ERROR;
  }
#else
  ...
  if (feof(fp)) {
    if (byteRead < reqlen) {
      error("Incomplete fread() of '%s' due to EOF : %" PRIu64
            " bytes read out of %" PRIu64 " requested",
            filename, byteRead, reqlen);
      return RIFF_READER_ERROR;
    }
    debug("fread() : EOF reached in file '%s'", filename);
  } else if (ferror(fp)) {
    if (byteRead < reqlen) {
      error("Incomplete fread() of '%s' due to error : %" PRIu64
            " bytes read out of %" PRIu64 " requested",
            filename, byteRead, reqlen);
    } else {
      error("fread() error of '%s' : %" PRIu64 " bytes read out of %" PRIu64
            " requested",
            filename, byteRead, reqlen);
    }
    return RIFF_READER_ERROR;
