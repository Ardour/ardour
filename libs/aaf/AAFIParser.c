...
static int parse_Mob(AAF_Iface *aafi, aafObject *Mob, td *__ptd);
...
static int parse_TimelineMobSlot(AAF_Iface *aafi, aafObject *TimelineMobSlot,
                                 td *__ptd);
static int parse_EventMobSlot(AAF_Iface *aafi, aafObject *EventMobSlot,
                              td *__ptd);
...
static int parse_DescriptiveMarker(AAF_Iface *aafi,
                                   aafObject *DescriptiveMarker, td *__ptd);
...
static int retrieve_UserComments(AAF_Iface *aafi, aafObject *UserComments,
                                 aafiMetaData **metadataList);
...
static int parse_CompositionMob(AAF_Iface *aafi, aafObject *CompoMob,
  ...
  aafUID_t *UsageCode =
      aaf_get_propertyValue(CompoMob, PID_Mob_UsageCode, &AAFTypeID_UsageType);

  if ((aafUIDCmp(aafi->aafd->Header.OperationalPattern,
                 &AAFOPDef_EditProtocol) &&
       aafUIDCmp(UsageCode, &AAFUsage_TopLevel)) ||
      (!aafUIDCmp(aafi->aafd->Header.OperationalPattern,
                  &AAFOPDef_EditProtocol) &&
       (aafUIDCmp(UsageCode, &AAFUsage_TopLevel) || !UsageCode))) {
    aafi->ctx.TopLevelCompositionMob = CompoMob;
    aafi->compositionName =
        aaf_get_propertyValue(CompoMob, PID_Mob_Name, &AAFTypeID_String);

    aafObject *UserComments =
        aaf_get_propertyValue(CompoMob, PID_Mob_UserComments,
                              &AAFTypeID_TaggedValueStrongReferenceVector);

    if (retrieve_UserComments(aafi, UserComments, &aafi->metadata) < 0) {
      TRACE_OBJ_WARNING(aafi, CompoMob, &__td,
                        "Error parsing Mob::UserComments");
    } else {
...
static int parse_SourceMob(AAF_Iface *aafi, aafObject *SourceMob, td *__ptd) {
  ...
  if (!EssenceDesc) {
    TRACE_OBJ_ERROR(aafi, SourceMob, &__td,
                    "Missing SourceMob::EssenceDescription");
    return -1;
  }
  ...
  if (aafi->ctx.current_audio_essence) {
    ...
    aafiAudioEssenceFile *audioEssenceFile =
        (aafiAudioEssenceFile *)aafi->ctx.current_audio_essence;

    memcpy(audioEssenceFile->umid, MobID, sizeof(aafMobID_t));

    int rc = snprintf(
        audioEssenceFile->originationDate,
        sizeof(((aafiAudioEssenceFile *)0)->originationDate), "%04u:%02u:%02u",
    ...
    assert(rc > 0 &&
           (size_t)rc < sizeof(((aafiAudioEssenceFile *)0)->originationDate));

    // if ( rc < 0 || (size_t)rc >= sizeof(((aafiAudioEssenceFile
    // *)0)->originationDate) ) { 	fprintf( stderr, "snprintf() error" ); 	return
    // -1;
    // }

    rc = snprintf(
        audioEssenceFile->originationTime,
        sizeof(((aafiAudioEssenceFile *)0)->originationTime), "%02u:%02u:%02u",
    ...
    assert(rc > 0 &&
           (size_t)rc < sizeof(((aafiAudioEssenceFile *)0)->originationTime));

    // if ( rc < 0 || (size_t)rc >= sizeof(((aafiAudioEssenceFile
    // *)0)->originationTime) ) { 	fprintf( stderr, "snprintf() error" ); 	return
    // -1;
    // }
  }
...
static int parse_TimelineMobSlot(AAF_Iface *aafi, aafObject *TimelineMobSlot,
  ...
  aafObject *ParentMob =
      aaf_get_ObjectAncestor(TimelineMobSlot, &AAFClassID_Mob);

  if (!ParentMob) {
    TRACE_OBJ_ERROR(aafi, TimelineMobSlot, &__td,
                    "Could not retrieve parent Mob");
    return -1;
  }

  uint32_t *track_num = aaf_get_propertyValue(
      TimelineMobSlot, PID_MobSlot_PhysicalTrackNumber, &AAFTypeID_UInt32);
  ...
  aafObject *Segment = aaf_get_propertyValue(
      TimelineMobSlot, PID_MobSlot_Segment, &AAFTypeID_SegmentStrongReference);
  ...
  aafWeakRef_t *dataDefWeakRef =
      aaf_get_propertyValue(Segment, PID_Component_DataDefinition,
                            &AAFTypeID_DataDefinitionWeakReference);

  if (!dataDefWeakRef) {
    TRACE_OBJ_ERROR(
        aafi, Segment, &__td,
        "Could not retrieve Component::DataDefinition from Segment child");
    return -1;
  }

  aafUID_t *DataDefinition =
      aaf_get_DataIdentificationByWeakRef(aafi->aafd, dataDefWeakRef);
  ...
  aafRational_t *edit_rate = aaf_get_propertyValue(
      TimelineMobSlot, PID_TimelineMobSlot_EditRate, &AAFTypeID_Rational);

  if (!edit_rate) {
    TRACE_OBJ_ERROR(aafi, TimelineMobSlot, &__td,
                    "Missing TimelineMobSlot::EditRate");
    return -1;
  }
  ...
  if (aafUIDCmp(ParentMob->Class->ID, &AAFClassID_CompositionMob)) {
    ...
    if (aafUIDCmp(DataDefinition, &AAFDataDef_Sound) ||
      ...
      if (aafi->ctx.TopLevelCompositionMob ==
          ParentMob /*!parentMobUsageCode || aafUIDCmp( parentMobUsageCode,
                       &AAFUsage_TopLevel )*/
          /*!aafi->ctx.is_inside_derivation_chain*/) {

        TRACE_OBJ(aafi, TimelineMobSlot, &__td);

        uint32_t tracknumber =
            (track_num) ? *track_num : (aafi->Audio->track_count + 1);

        aafiAudioTrack *track = aafi_newAudioTrack(aafi);

        track->number = tracknumber;
        track->name = aaf_get_propertyValue(
            TimelineMobSlot, PID_MobSlot_SlotName, &AAFTypeID_String);
        ...
        void *TimelineMobAttributeList = aaf_get_propertyValue(
            TimelineMobSlot,
            aaf_get_PropertyIDByName(aafi->aafd, "TimelineMobAttributeList"),
            &AAFTypeID_TaggedValueStrongReferenceVector);

        if (TimelineMobAttributeList) {

          int32_t *solo =
              aaf_get_TaggedValueByName(aafi->aafd, TimelineMobAttributeList,
                                        "AudioMixerCompSolo", &AAFTypeID_Int32);
          int32_t *mute =
              aaf_get_TaggedValueByName(aafi->aafd, TimelineMobAttributeList,
                                        "AudioMixerCompMute", &AAFTypeID_Int32);
      ...
      track->number = tracknumber;
      track->name = aaf_get_propertyValue(TimelineMobSlot, PID_MobSlot_SlotName,
                                          &AAFTypeID_String);
      track->edit_rate = edit_rate;
      ...
      /*
       * Avid Media Composer 23.12, markers with duration (no-duration markers
are held by AAFClassID_EventMobSlot)
       *
│ 04179│     └──◻ AAFClassID_TimelineMobSlot [slot:3003 track:12] (DataDef:
AAFDataDef_DescriptiveMetadata) : Segmentation │ 01946│          └──◻
AAFClassID_Sequence (Length: 4) │ 03031│               └──◻
AAFClassID_DescriptiveMarker (Length: 4)   (MetaProps:
CommentMarkerAttributeList[0xffd4] CommentMarkerColor[0xffdb]
CommentMarkerUSer[0xffda] CommentMarkerDate[0xffd9] CommentMarkerTime[0xffd8])
       */
    ...
    if (aafi->ctx.current_audio_essence) {
      ...
      aafPosition_t *Origin = aaf_get_propertyValue(
          TimelineMobSlot, PID_TimelineMobSlot_Origin, &AAFTypeID_PositionType);

      if (!Origin) {
        TRACE_OBJ_ERROR(aafi, TimelineMobSlot, &__td,
                        "Missing TimelineMobSlot::Origin");
        return -1;
      }
    ...
    /*
     * AAFClassID_MasterMob and AAFClassID_SourceMob are accessed directly from
     * TimelineMobSlot > SourceClip
     */
    TRACE_OBJ_NO_SUPPORT(aafi, TimelineMobSlot, &__td);
    return -1;
  }
...
static int parse_EventMobSlot(AAF_Iface *aafi, aafObject *EventMobSlot,
  ...
  aafRational_t *edit_rate = aaf_get_propertyValue(
      EventMobSlot, PID_EventMobSlot_EditRate, &AAFTypeID_Rational);

  if (!edit_rate) {
    TRACE_OBJ_ERROR(aafi, EventMobSlot, &__td,
                    "Missing EventMobSlot::EditRate");
    return -1;
  }
...
static int parse_Transition(AAF_Iface *aafi, aafObject *Transition, td *__ptd) {
  ...
  aafWeakRef_t *dataDefWeakRef =
      aaf_get_propertyValue(Transition, PID_Component_DataDefinition,
                            &AAFTypeID_DataDefinitionWeakReference);

  if (!dataDefWeakRef) {
    TRACE_OBJ_ERROR(aafi, Transition, &__td,
                    "Missing Component::DataDefinition.");
    return -1;
  }
  ...
  if (!aafUIDCmp(DataDefinition, &AAFDataDef_Sound) &&
    ...
    TRACE_OBJ_ERROR(
        ...
        "Current implementation does not support video Transitions");
    return -1;
  }
  ...
  if (!OpGroup) {
    TRACE_OBJ_ERROR(aafi, Transition, &__td,
                    "Missing Transition::OperationGroup");
    return -1;
  }

  aafPosition_t *cutPoint = aaf_get_propertyValue(
      Transition, PID_Transition_CutPoint, &AAFTypeID_PositionType);

  if (!cutPoint) {
    /* not encountered though */
    debug("Missing Transition::CutPoint : setting cut point to "
          "Transition::Length/2");
  }
   ...
   * the fade curve. However, this parameter is optional regarding
   * AAF_EditProtocol and there is most likely no implementation that exports
   * custom fade curves. Thus, we only retrieve ParameterDef_Level to possibly
   * set interpolation, and we always set the fade as defined in
   * AAF_EditProtocol, with only two points :
   *
  ...
  if (fadeType & AAFI_TRANS_FADE_IN || fadeType & AAFI_TRANS_XFADE) {
...
static int parse_Filler(AAF_Iface *aafi, aafObject *Filler, td *__ptd) {
  ...
  aafWeakRef_t *dataDefWeakRef =
      aaf_get_propertyValue(Filler, PID_Component_DataDefinition,
                            &AAFTypeID_DataDefinitionWeakReference);
...
static int parse_SourceClip(AAF_Iface *aafi, aafObject *SourceClip, td *__ptd) {
  ...
  aafWeakRef_t *dataDefWeakRef =
      aaf_get_propertyValue(SourceClip, PID_Component_DataDefinition,
                            &AAFTypeID_DataDefinitionWeakReference);

  if (!dataDefWeakRef) {
    TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
                    "Missing Component::DataDefinition.");
    return -1;
  }
  ...
  aafUID_t *parentMobUsageCode =
      aaf_get_propertyValue(ParentMob, PID_Mob_UsageCode, &AAFTypeID_UsageType);
  ...
  if (!SourceMobSlotID) {
    TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
                    "Missing SourceReference::SourceMobSlotID");
    return -1;
  }

  /*
   * TODO: handle SourceReference::ChannelIDs and
   * SourceReference::MonoSourceSlotIDs (Multi-channels)
   */
  ...
  if (sourceID == NULL || aafMobIDCmp(sourceID, &AAFMOBID_NULL)) {
    ...
    if (!targetMob) {
      TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
                      "Could not retrieve target Mob by ID : %s",
                      aaft_MobIDToText(sourceID));
      return -1;
    }

    aafObject *targetMobSlots = aaf_get_propertyValue(
        targetMob, PID_Mob_Slots, &AAFTypeID_MobSlotStrongReferenceVector);
    ...
    if (!targetMobSlot) {
       ...
       * valgrind --track-origins=yes --leak-check=full ./bin/AAFInfo --trace
       * --aaf-essences  --aaf-clips
       * /home/agfline/Programming/libaaf/LibAAF/test/private/aaf/ADP/ADP_STTRACK_CLIPGAIN_TRACKGAIN_XFADE_NOOPTONEXPORT.aaf
       * --verb 3
       */
      TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
  ...
  (AAFOperationDef_AudioChannelCombiner)
           * We already parsed the first SourceClip in
  AAFOperationDef_AudioChannelCombiner.
           * We just have to check everything match for all clips left (each
  clip represents a channel)
           *
  │ 02277│               ├──◻ AAFClassID_OperationGroup (OpIdent:
  AAFOperationDef_AudioChannelCombiner; Length: 517207) │ 02816│               │
  ├──◻ AAFClassID_SourceClip (Length: 516000) │ 02821│               │    │ └──◻
  AAFClassID_MasterMob (UsageCode: n/a) : 7 │ 04622│               │    │ └──◻
  AAFClassID_TimelineMobSlot [slot:1 track:1] (DataDef : AAFDataDef_Sound) │
  03185│               │    │              └──◻ AAFClassID_SourceClip (Length:
  523723) │ 04346│               │    │                   └──◻
  AAFClassID_SourceMob (UsageCode: n/a) : 7 │ 01393│               │    │ └──◻
  AAFClassID_WAVEDescriptor │ 01532│               │    │ └──◻
  AAFClassID_NetworkLocator :
  file://localhost/C:/Users/user/Desktop/res/7.1-SMPTE-channel-identif.wav │ │
  ...
  │ 02666│               │    └──◻ AAFClassID_SourceClip (Length: 516000)
          ...
          if (targetMob &&
              !aafUIDCmp(targetMob->Class->ID, &AAFClassID_MasterMob)) {
            TRACE_OBJ_ERROR(
                aafi, SourceClip, &__td,
                "Multichannel-combined SourceClip does not target a MasterMob: "
                "%s",
                aaft_ClassIDToText(aafi->aafd, targetMob->Class->ID));
            return -1;
          }

          if (aafMobIDCmp(aafi->ctx.current_clip->essencePointerList
                              ->essenceFile->masterMobID,
                          sourceID) &&
              aafi->ctx.current_clip->essencePointerList->essenceFile
                      ->masterMobSlotID == *SourceMobSlotID) {
            /*
             * Clip channel rely on the same audio file source (single
             * multichannel file)
             *
             * Assume that all clip channels will point to the same multichannel
             * essence file, in the right order. (Davinci Resolve multichannel
             * clips)
             */

            aafi->ctx.current_clip->essencePointerList->essenceChannel = 0;

            TRACE_OBJ_INFO(aafi, SourceClip, &__td,
                           "Ignore parsing of clip channel %i pointing to the "
                           "same audio source file",
                           aafi->ctx.current_combined_clip_channel_num + 1);
            return 0;
          }
        }

        if (!aafi->ctx.current_clip_is_combined ||
            (aafi->ctx.current_clip_is_combined &&
             aafi->ctx.current_combined_clip_channel_num == 0)) {
          /*
          ...
          aafiAudioClip *audioClip =
              aafi_newAudioClip(aafi, aafi->ctx.current_track);
          ...
          if (aafi->ctx.avid_warp_clip_edit_rate) {
            audioClip->essence_offset =
                aafi_convertUnit(*startTime, aafi->ctx.avid_warp_clip_edit_rate,
                                 aafi->ctx.current_track->edit_rate);
            audioClip->len = aafi_convertUnit(
                (aafi->ctx.current_combined_clip_forced_length)
                    ? aafi->ctx.current_combined_clip_forced_length
                    : *length,
                aafi->ctx.avid_warp_clip_edit_rate,
                aafi->ctx.current_track->edit_rate);
          } else {
          ...
          /*
           * ComponentAttributeList is non-standard, but used by Avid Media
           * Composer and Davinci Resolve to attach Clip Notes.
           */

          void *ComponentAttributeList = aaf_get_propertyValue(
              SourceClip,
              aaf_get_PropertyIDByName(aafi->aafd, "ComponentAttributeList"),
              &AAFTypeID_TaggedValueStrongReferenceVector);

          if (ComponentAttributeList) {

            char *comment =
                aaf_get_TaggedValueByName(aafi->aafd, ComponentAttributeList,
                                          "_COMMENT", &AAFTypeID_String);
        ...
        /* Exemple of AAFUsage_SubClip in
../test/private/aaf/AAF/E2R_52min/224E2R190008_ENQUETES_DE_REGION_N32_LA_SECHERESSE_UNE_FATALITE.aaf
         * TODO: Looks like a sub-clip is just used to reference an existing
clip, but to show it in UI with a different name.
         * It seems that sub-clip length always matches MasterMob > SourceLip
length
         * Therefore, we should only parse its name.
         *
│ 02709││         ├──◻ AAFClassID_SourceClip (Length: 183)
│ 02723││         │    └──◻ AAFClassID_CompositionMob (UsageCode:
AAFUsage_SubClip) : plato 8 1ere.Copy.01.new.06  (MetaProps:
SubclipBegin[0xfff6] SubclipFullLength[0xfff7] MobAttributeList[0xfff9]) │
04478││         │         └──◻ AAFClassID_TimelineMobSlot [slot:2 track:2]
(DataDef : AAFDataDef_Sound) │ 02978││         │              └──◻
AAFClassID_SourceClip (Length: 681) │ 02983││         │                   └──◻
AAFClassID_MasterMob (UsageCode: n/a) : FTVHD-X400-0946.new.06  (MetaProps:
MobAttributeList[0xfff9] ConvertFrameRate[0xfff8]) │ 04561││         │ └──◻
AAFClassID_TimelineMobSlot [slot:2 track:2] (DataDef : AAFDataDef_Sound) :
FTVHD-X400-0946 │ 03188││         │                             └──◻
AAFClassID_SourceClip (Length: 681) │ 04305││         │ └──◻
AAFClassID_SourceMob (UsageCode: n/a) : FTVHD-X400-0946.PHYS  (MetaProps:
MobAttributeList[0xfff9]) │ 01342││         │ └──◻ AAFClassID_PCMDescriptor
(MetaProps: DataOffset[0xffde])
         */

        aafi->ctx.current_clip->subClipName =
        ...
        aafObject *UserComments =
            aaf_get_propertyValue(ParentMob, PID_Mob_UserComments,
                                  &AAFTypeID_TaggedValueStrongReferenceVector);

        if (retrieve_UserComments(aafi, UserComments,
                                  &aafi->ctx.current_clip->metadata) < 0) {
        ...
        // 	applyGainOffset( aafi, &aafi->ctx.current_clip->gain,
        // aafi->ctx.current_adjusted_clip_gain ); 	aafi_freeAudioGain(
        // aafi->ctx.current_adjusted_clip_gain );
        // 	aafi->ctx.current_adjusted_clip_gain = NULL;
        // }
      } else if (!parentMobUsageCode) {
        debug("CompositionMob UsageCode is NULL. Keep on parsing...");
      } else {
        debug("Unsupported CompositionMob UsageCode: %s",
              aaft_UsageCodeToText(parentMobUsageCode));
      ...
      if (targetMob && aafUIDCmp(targetMob->Class->ID, &AAFClassID_MasterMob)) {
        ...
        parse_MobSlot(aafi, targetMobSlot, &__td);
                 ...
                 aafUIDCmp(targetMob->Class->ID, &AAFClassID_CompositionMob)) {
      ...
      if (aafi->ctx.TopLevelCompositionMob != ParentMob) {
        TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
                        "Current implementation does not support parsing video "
                        "SourceClip out of TopLevel CompositionMob");
        return -1;
      }
      ...
      if (targetMob && aafUIDCmp(targetMob->Class->ID, &AAFClassID_MasterMob)) {
        ...
        TRACE_OBJ_ERROR(
            aafi, SourceClip, &__td,
            "Current implementation does not support video SourceClip not "
            "targetting a MasterMob: %s",
            (targetMob) ? aaft_ClassIDToText(aafi->aafd, targetMob->Class->ID)
                        : "[MISSING TARGET MOB]");
        return -1;
      }
    }
  ...
  /* *** Essence *** */
    ...
    aafObject *ParentMobSlot =
        aaf_get_ObjectAncestor(SourceClip, &AAFClassID_MobSlot);
    ...
    uint32_t *essenceChannelNum = aaf_get_propertyValue(
        ParentMobSlot, PID_MobSlot_PhysicalTrackNumber, &AAFTypeID_UInt32);
    ...
    if (aafUIDCmp(DataDefinition, &AAFDataDef_Sound) ||
      ...
      AAFI_foreachAudioEssenceFile(aafi, audioEssenceFile) {
        if (aafMobIDCmp(audioEssenceFile->sourceMobID, sourceID) &&
            audioEssenceFile->sourceMobSlotID == (unsigned)*SourceMobSlotID) {
          __td.eob = 1;
          TRACE_OBJ_INFO(aafi, SourceClip, &__td,
                         "Essence already parsed: Linking with %s",
                         audioEssenceFile->name);
          aafi->ctx.current_clip->essencePointerList =
              aafi_newAudioEssencePointer(
                  aafi, &aafi->ctx.current_clip->essencePointerList,
                  audioEssenceFile, essenceChannelNum);
          return 0;
        }
      }
      ...
      aafObject *SourceMob =
          aaf_get_MobByID(aafi->aafd->Mobs, audioEssenceFile->sourceMobID);

      if (!SourceMob) {
        TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
                        "Could not retrieve SourceMob by ID : %s",
                        aaft_MobIDToText(audioEssenceFile->sourceMobID));
        return -1;
      }

      aafi->ctx.current_audio_essence = audioEssenceFile;

      void *MobUserComments =
          aaf_get_propertyValue(ParentMob, PID_Mob_UserComments,
                                &AAFTypeID_TaggedValueStrongReferenceVector);

      if (retrieve_UserComments(aafi, MobUserComments,
                                &audioEssenceFile->metadata) < 0) {
        TRACE_OBJ_WARNING(aafi, SourceClip, &__td,
                          "Error parsing parent Mob::UserComments");
      } else {
        TRACE_OBJ(aafi, SourceClip, &__td);
      }
      ...
      aafObject *EssenceData =
          aaf_get_EssenceDataByMobID(aafi->aafd, audioEssenceFile->sourceMobID);
      ...
      aafi->ctx.current_clip->essencePointerList = aafi_newAudioEssencePointer(
          aafi, &aafi->ctx.current_clip->essencePointerList, audioEssenceFile,
          essenceChannelNum);
      ...
      AAFI_foreachVideoEssence(aafi, videoEssenceFile) {
        if (aafMobIDCmp(videoEssenceFile->sourceMobID, sourceID) &&
            videoEssenceFile->sourceMobSlotID == (unsigned)*SourceMobSlotID) {
          __td.eob = 1;
          TRACE_OBJ_INFO(aafi, SourceClip, &__td,
                         "Essence already parsed: Linking with %s",
      ...
      aafObject *SourceMob =
          aaf_get_MobByID(aafi->aafd->Mobs, videoEssenceFile->sourceMobID);

      if (!SourceMob) {
        TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
                        "Could not retrieve SourceMob by ID : %s",
                        aaft_MobIDToText(videoEssenceFile->sourceMobID));
        return -1;
      }
      ...
      aafObject *EssenceData =
          aaf_get_EssenceDataByMobID(aafi->aafd, videoEssenceFile->sourceMobID);
      ...
      /*
       * No need to check for uniqueness, current version supports only one
       * video clip.
       */
      videoEssenceFile->unique_name =
          laaf_util_c99strdup(videoEssenceFile->name);

      if (!videoEssenceFile->unique_name) {
        TRACE_OBJ_ERROR(aafi, SourceClip, &__td,
                        "Could not duplicate video essence unique name : %s",
                        videoEssenceFile->name);
        return -1;
      }
    ...
    /* Nothing to parse here at first glance : SourceMob > TimelineMobSlot >
     * SourceClip */
    TRACE_OBJ(aafi, SourceClip, &__td);
  } else {
    TRACE_OBJ_NO_SUPPORT(aafi, SourceClip, &__td);
...
static int parse_Timecode(AAF_Iface *aafi, aafObject *Timecode, td *__ptd) {
  ...
  aafObject *ParentMobSlot =
      aaf_get_ObjectAncestor(Timecode, &AAFClassID_MobSlot);
  ...
  if (tc_edit_rate == NULL) {
    TRACE_OBJ_ERROR(aafi, Timecode, &__td,
                    "Missing parent TimelineMobSlot::EditRate");
    return -1;
  }

  if (aafi->Timecode) {
    TRACE_OBJ_WARNING(aafi, Timecode, &__td,
...
static int parse_DescriptiveMarker(AAF_Iface *aafi,
  ...
  if (!start) {
    /*
     * « If an Event is in a TimelineMobSlot or a StaticMobSlot, it shall not
     * have a Position property. If an Event is in a EventMobSlot, it shall have
     * a Position property. »
     */
  ...
  char *comment = aaf_get_propertyValue(DescriptiveMarker, PID_Event_Comment,
                                        &AAFTypeID_String);
  char *name = aaf_get_propertyValue(
      DescriptiveMarker,
      aaf_get_PropertyIDByName(aafi->aafd, "CommentMarkerUser"),
      &AAFTypeID_String);

  if (!name) {
    /* Avid Media Composer 23.12 */
    name = aaf_get_propertyValue(
        DescriptiveMarker,
        aaf_get_PropertyIDByName(aafi->aafd, "CommentMarkerUSer"),
        &AAFTypeID_String);
  }
  ...
  aafProperty *RGBColorProp = aaf_get_property(
      DescriptiveMarker,
      aaf_get_PropertyIDByName(aafi->aafd, "CommentMarkerColor"));
...
static int parse_Sequence(AAF_Iface *aafi, aafObject *Sequence, td *__ptd) {
   ...
   *
   * "Audio Warp" OperationGroup is pointing to a Sequence with two
ComponentAttributes:
   *    _MIXMATCH_RATE_NUM
   *    _MIXMATCH_RATE_DENOM
   *
   * Those parameters set the valid edit rate for SourceClip::StartTime (essence
   * offset) and Component::Length, in total violation of the standard saying
that
   * TimelineMobSlot::EditRate shall always be used.
   *
│ 02009││    │         ├──◻ AAFClassID_OperationGroup (OpIdent: Audio Warp;
Length: 168)   (MetaProps: ComponentAttributeList[0xffc9]) │ 02283││    │ │ ├──◻
AAFClassID_ConstantValue (ParamDef: AvidMotionInputFormat; Type:
AAFTypeID_Int32) : 2 │ 02283││    │         │    ├──◻ AAFClassID_ConstantValue
(ParamDef: AAFParameterDef_SpeedRatio; Type: AAFTypeID_Rational) : 1/2 │ 02283││
│         │    ├──◻ AAFClassID_ConstantValue (ParamDef: AvidPhase; Type:
AAFTypeID_Int32) : 0 │ 02283││    │         │    ├──◻ AAFClassID_ConstantValue
(ParamDef: AvidMotionOutputFormat; Type: AAFTypeID_Int32) : 0 │ 02283││    │ │
└──◻ AAFClassID_ConstantValue (ParamDef: AvidMotionPulldown; Type:
AAFTypeID_Int32) : 0 │ 01628││    │         │    └──◻ AAFClassID_Sequence
(Length: 336)  (MetaProps: ComponentAttributeList[0xffc9]) │ 01195││    │ │ └──◻
AAFClassID_SourceClip (Length: 336) │ 01198││    │         │              └──◻
AAFClassID_MasterMob (UsageCode: n/a) :
UTS_LIMO_LIMO_3_2024_01_30_11_15_45.new.04  (MetaProps: MobAttributeList[0xfff9]
ConvertFrameRate[0xfff8]) │ 00513││    │         │                   └──◻
AAFClassID_TimelineMobSlot [slot:1 track:1] (DataDef: AAFDataDef_Sound) │
01628││    │         │                        └──◻ AAFClassID_Sequence (Length:
576) │ 01359││    │         │                             └──◻
AAFClassID_SourceClip (Length: 576) │ 00272││    │         │ ├──◻
AAFClassID_SourceMob (UsageCode: n/a)   (MetaProps: MobAttributeList[0xfff9]) │
02806││    │         │                                  │    ├──◻
AAFClassID_WAVEDescriptor (ContainerIdent : AAFContainerDef_AAF) │ 03010││    │
│                                  │    │    └──◻ AAFClassID_NetworkLocator
(URLString:
file://10.87.230.71/mixage/AAF_Vers_Mixage/MONTAGE_06/4550985%20SUIVI%20AGRICULTEURS%20VERS%20PARIS.aaf)
│      ││    │         │                                  │    │
│ 00532││    │         │                                  │    └──◻
AAFClassID_TimelineMobSlot [slot:1 track:1] (DataDef: AAFDataDef_Sound) │
01470││    │         │                                  │         └──◻
AAFClassID_SourceClip (Length: 576) │ 03071││    │         │ └──◻
AAFClassID_EssenceData (Data: Data-2702)

   */

  void *ComponentAttributeList = aaf_get_propertyValue(
      Sequence, aaf_get_PropertyIDByName(aafi->aafd, "ComponentAttributeList"),
      &AAFTypeID_TaggedValueStrongReferenceVector);

  if (ComponentAttributeList) {
    int32_t *rateNum =
        aaf_get_TaggedValueByName(aafi->aafd, ComponentAttributeList,
                                  "_MIXMATCH_RATE_NUM", &AAFTypeID_Int32);
    int32_t *rateDenom =
        aaf_get_TaggedValueByName(aafi->aafd, ComponentAttributeList,
                                  "_MIXMATCH_RATE_DENOM", &AAFTypeID_Int32);
...
static int parse_Selector(AAF_Iface *aafi, aafObject *Selector, td *__ptd) {
  ...
  void *ComponentAttributeList = aaf_get_propertyValue(
      Selector, aaf_get_PropertyIDByName(aafi->aafd, "ComponentAttributeList"),
      &AAFTypeID_TaggedValueStrongReferenceVector);

  if (ComponentAttributeList) {
    int32_t *disabledClip =
        aaf_get_TaggedValueByName(aafi->aafd, ComponentAttributeList,
                                  "_DISABLE_CLIP_FLAG", &AAFTypeID_Int32);
...
static int parse_NestedScope(AAF_Iface *aafi, aafObject *NestedScope,
  ...
  aafObject *Slot = NULL;
  aafObject *Slots =
      aaf_get_propertyValue(NestedScope, PID_NestedScope_Slots,
                            &AAFTypeID_SegmentStrongReferenceVector);
...
static int parse_OperationGroup(AAF_Iface *aafi, aafObject *OpGroup,
  ...
  if (!aafUIDCmp(ParentMob->Class->ID, &AAFClassID_CompositionMob)) {
    TRACE_OBJ_ERROR(aafi, OpGroup, &__td,
                    "OperationGroup is currently supported only in "
                    "CompositionMob, not in %s",
                    aaft_ClassIDToText(aafi->aafd, ParentMob->Class->ID));
    return -1;
  }
  ...
  aafWeakRef_t *OperationDefWeakRef =
      aaf_get_propertyValue(OpGroup, PID_OperationGroup_Operation,
  ...
  aafUID_t *OperationIdentification =
      aaf_get_OperationIdentificationByWeakRef(aafi->aafd, OperationDefWeakRef);
  ...
  /*
   * We need to check if OperationGroup is a direct child of
   * TopLevelCompositionMob > TimelineMobSlot.
   *  - If it is, it means that the OperationGroup affect the current Track.
   *  - If it's not (eg. it's a child of a Sequence), then OperationGroup
   * applies to all descendent clips.
   *
  ...
  while (OperationGroupParent && aafUIDCmp(OperationGroupParent->Class->ID,
  ...
  if (aafUIDCmp(OperationIdentification, &AAFOperationDef_MonoAudioDissolve)) {
    ...
    if (!aafUIDCmp(OpGroup->Parent->Class->ID, &AAFClassID_Transition)) {
      TRACE_OBJ_ERROR(aafi, OpGroup, &__td,
                      "Parent should be AAFClassID_Transition");
      return -1;
    }
    ...
    /*
     * Avid Media Composer doesn't use the standard method to set interpolation.
     * Instead, it always sets InterpolationIdentification to Linear, and it
     * sets the actual interpolation in both :
     * 	- OperationGroup > ComponentAttributeList >
     * _ATN_AUDIO_DISSOLVE_CURVETYPE
     * 	- OperationGroup > Parameters > AAFClassID_ConstantValue (ParamDef:
     * Curve Type; Type: AAFTypeID_Int32) : 1
     *
     * Note: _ATN_AUDIO_DISSOLVE_CURVETYPE was observed since v8.4 (2015),
     * however "Curve Type" was observed since v18.12.7. Using
     * _ATN_AUDIO_DISSOLVE_CURVETYPE provides a better support for older Avid MC
     * versions.
     */

    void *ComponentAttributeList = aaf_get_propertyValue(
        OpGroup, aaf_get_PropertyIDByName(aafi->aafd, "ComponentAttributeList"),
        &AAFTypeID_TaggedValueStrongReferenceVector);

    if (ComponentAttributeList) {

      int32_t *curveType = aaf_get_TaggedValueByName(
          aafi->aafd, ComponentAttributeList, "_ATN_AUDIO_DISSOLVE_CURVETYPE",
          &AAFTypeID_Int32);
    ...
    if (resolve_AAF(aafi)) {
      /*
       * This is clearly a violation of the standard (p 57). When Davinci
       * Resolve exports multichannel clips, it does not set SourceClip::Length
       * correctly. Instead, it's more like some sort of frame-rounded value
       * which doesn't match the timeline. However, the correct value is set to
       * OperationGroup::length...
       */
      int64_t *length = aaf_get_propertyValue(OpGroup, PID_Component_Length,
                                              &AAFTypeID_LengthType);
    ...
    if (aafi->ctx.current_combined_clip_total_channel == 2) {
      ...
      TRACE_OBJ_ERROR(aafi, OpGroup, &__td, "Unknown track format (%u)",
                      aafi->ctx.current_combined_clip_total_channel);
    ...
    if (!Parameters) {
      TRACE_OBJ_ERROR(aafi, OpGroup, &__td,
                      "Missing OperationGroup::Parameters");
      rc = -1;
      goto end; /* we still have to parse InputSegments, so we're not losing any
                   clip */
    }
    ...
    AAFI_foreach_ObjectInSet(&Param, Parameters, i, __td) {
      parse_Parameter(aafi, Param, &__td);
                 ...
                 aaf_get_OperationDefIDByName(aafi->aafd, "Audio Warp"))) {
     ...
     */
    debug("Unsupported OperationIdentification: %s",
    ...
    if (!Parameters) {
      // TRACE_OBJ_ERROR( aafi, OpGroup, &__td, "Missing
      // OperationGroup::Parameters" );
      rc = -1;
      goto end; /* we still have to parse InputSegments, so we're not losing any
                   clip */
    }
...
static int parse_Parameter(AAF_Iface *aafi, aafObject *Parameter, td *__ptd) {
  ...
  if (aafUIDCmp(Parameter->Class->ID, &AAFClassID_ConstantValue)) {
    ...
    __td_set(__td, __ptd, 1);
    TRACE_OBJ_ERROR(aafi, Parameter, &__td,
                    "Parameter is neither of class Constant nor Varying : %s",
                    aaft_ClassIDToText(aafi->aafd, Parameter->Class->ID));
  }
...
static int parse_ConstantValue(AAF_Iface *aafi, aafObject *ConstantValue,
  ...
  if (!ParamDef) {
    TRACE_OBJ_ERROR(aafi, ConstantValue, &__td,
                    "Missing Parameter::Definition");
    return -1;
  }

  aafWeakRef_t *OperationDefWeakRef =
      aaf_get_propertyValue(ConstantValue->Parent, PID_OperationGroup_Operation,
                            &AAFTypeID_OperationDefinitionWeakReference);

  if (!OperationDefWeakRef) {
    TRACE_OBJ_ERROR(aafi, ConstantValue, &__td,
                    "Missing OperationGroup::Operation");
    return -1;
  }
  ...
  if (!OperationIdentification) {
    TRACE_OBJ_ERROR(aafi, ConstantValue, &__td,
                    "Could not retrieve OperationIdentification from parent");
    return -1;
  }
  ...
  if (aafUIDCmp(OperationIdentification, &AAFOperationDef_MonoAudioGain) &&
    ...
    if (!value) {
      TRACE_OBJ_ERROR(
          ...
          "Could not retrieve Indirect value for ConstantValue::Value");
      return -1;
    }

    aafiAudioGain *Gain =
        aafi_newAudioGain(aafi, AAFI_AUDIO_GAIN_CONSTANT, 0, value);
    ...
    if (aafUIDCmp(mobUsageCode, &AAFUsage_AdjustedClip)) {
       ...
       *
       * Only Avid Media Composer seems to make use of AdjustedClip, in a way
       * that it doesn't affect the timeline composition. It looks like any gain
       * applied to a source clip (inside a bin), is just audible when playing
       * that clip in preview.
       *
       * Thus, we just ignore it.
       */
      debug("Ignoring AdjustedClip audio level: %i/%i (%+05.1lf dB) ",
            Gain->value[0].numerator, Gain->value[0].denominator,
            20 * log10(aafRationalToDouble(Gain->value[0])));
      TRACE_OBJ_WARNING(aafi, ConstantValue, &__td,
                        "Ignoring AdjustedClip audio level");
      ...
      if (!aafi->ctx.current_track) {
        TRACE_OBJ_ERROR(
            aafi, ConstantValue, &__td,
            "Current track not set, dropping this volume: %i/%i (%+05.1lf dB)",
            Gain->value[0].numerator, Gain->value[0].denominator,
            20 * log10(aafRationalToDouble(Gain->value[0])));
        aafi_freeAudioGain(Gain);
        return -1;
        ...
        TRACE_OBJ_ERROR(aafi, ConstantValue, &__td,
                        "Track volume was already set, dropping this one: "
                        "%i/%i (%+05.1lf dB)",
                        Gain->value[0].numerator, Gain->value[0].denominator,
                        20 * log10(aafRationalToDouble(Gain->value[0])));
        aafi_freeAudioGain(Gain);
      ...
      /*
       * Clip-based Gain
       * Gain is saved in context and it will be set to all OperationGroup
       * descendent clips.
       */
      if (aafi->ctx.current_clip_gain) {
        TRACE_OBJ_ERROR(
            aafi, ConstantValue, &__td,
            "Clip gain was already set, dropping this one: %i/%i (%+05.1lf dB)",
            Gain->value[0].numerator, Gain->value[0].denominator,
            20 * log10(aafRationalToDouble(Gain->value[0])));
        aafi_freeAudioGain(Gain);
        return -1;
    ...
    if (!aafi->ctx.current_opgroup_affect_track) {
      /*
       * « Pan automation shall be track-based. If an application has a
       * different native representation (e.g., clip-based pan), it shall
       * convert to and from its native representation when exporting and
    ...
    aafiAudioPan *Pan =
        aafi_newAudioPan(aafi, AAFI_AUDIO_GAIN_CONSTANT, 0, value);
...
static int parse_VaryingValue(AAF_Iface *aafi, aafObject *VaryingValue,
  ...
  aafWeakRef_t *OperationDefWeakRef =
      aaf_get_propertyValue(VaryingValue->Parent, PID_OperationGroup_Operation,
                            &AAFTypeID_OperationDefinitionWeakReference);

  if (!OperationDefWeakRef) {
    TRACE_OBJ_ERROR(aafi, VaryingValue, &__td,
                    "Missing OperationGroup::Operation");
    return -1;
  }
  ...
  aafWeakRef_t *InterpolationDefWeakRef =
      aaf_get_propertyValue(VaryingValue, PID_VaryingValue_Interpolation,
                            &AAFTypeID_InterpolationDefinitionWeakReference);

  if (!InterpolationDefWeakRef) {
    TRACE_OBJ_ERROR(aafi, VaryingValue, &__td,
                    "Missing VaryingValue::Interpolation.");
    return -1;
  }

  aafiInterpolation_e interpolation = 0;
  aafUID_t *InterpolationIdentification =
      aaf_get_InterpolationIdentificationByWeakRef(aafi->aafd,
                                                   InterpolationDefWeakRef);

  if (!InterpolationIdentification) {
    TRACE_OBJ_WARNING(aafi, VaryingValue, &__td,
                      "Could not retrieve InterpolationIdentification: Falling "
                      "back to Linear");
    ...
    TRACE_OBJ_WARNING(
        ...
        "Unknown InterpolationIdentification value: Falling back to Linear");
    interpolation = AAFI_INTERPOL_LINEAR;
  }
  ...
  if (!Points) {
     ...
     */
    TRACE_OBJ_WARNING(aafi, VaryingValue, &__td,
                      "Missing VaryingValue::PointList");
    return -1;
  }
  ...
  if (aafUIDCmp(OperationIdentification, &AAFOperationDef_MonoAudioDissolve) &&
    ...
    if (!Trans) {
      TRACE_OBJ_ERROR(aafi, VaryingValue, &__td,
                      "Context current_transition not set");
      return -1;
    }
    ...
    /*
     * OperationGroup *might* contain a Parameter (ParameterDef_Level)
     * specifying the fade curve. However, this parameter is optional regarding
     * AAF_EditProtocol and there is most likely no implementation that exports
     * custom fade curves. Thus, we only retrieve ParameterDef_Level to set
     * interpolation, and we always set the fade as defined in AAF_EditProtocol,
    ...
    TRACE_OBJ(aafi, VaryingValue, &__td);
                       ...
                       &AAFOperationDef_MonoAudioGain) &&
             (aafUIDCmp(ParamDef, &AAFParameterDef_Amplitude) ||
              aafUIDCmp(ParamDef, aaf_get_ParamDefIDByName(
                                      aafi->aafd, "AvidControlClipRatio")))) {
    ...
    if (Gain->pts_cnt == 2 &&
      ...
      if (aafRationalToDouble(Gain->value[0]) == 1.0f) {
        /*
         * Skipping any 1:1 gain allows not to miss any other actual gain (eg.
         DR_Audio_Levels.aaf, Resolve 18.5.AAF)
         *
             │ 02412││         ├──◻ AAFClassID_OperationGroup (OpIdent:
         AAFOperationDef_MonoAudioGain; Length: 284630) err │ 03839││         │
         ├──◻ AAFClassID_VaryingValue : : Value is continuous 1:1 (0db),
         skipping it. │      ││         │    │ │ 02412││         │    └──◻
         AAFClassID_OperationGroup (OpIdent: AAFOperationDef_MonoAudioGain;
         Length: 284630) │ 03660││         │         ├──◻
         AAFClassID_ConstantValue (Value: 6023/536870912  -99.0 dB) │ 02983││ │
         └──◻ AAFClassID_SourceClip (Length: 284630) │ 02988││         │ └──◻
         AAFClassID_MasterMob (UsageCode: n/a) : speech-sample.mp3 - -100db │
         04553││         │                   └──◻ AAFClassID_TimelineMobSlot
         [slot:1 track:1] (DataDef : AAFDataDef_Sound) │ 03193││         │ └──◻
         AAFClassID_SourceClip (Length: 284630) │ 04297││         │ └──◻
         AAFClassID_SourceMob (UsageCode: n/a) : speech-sample.mp3 - -100db │
         01342││         │                                  └──◻
         AAFClassID_PCMDescriptor │ 01529││         │ └──◻
         AAFClassID_NetworkLocator :
         file:///C:/Users/user/Desktop/libAAF/test/res/speech-sample.mp3
         */
        TRACE_OBJ_INFO(aafi, VaryingValue, &__td,
                       "Value is continuous 1:1 (0db), skipping it.");
        aafi_freeAudioGain(Gain);
        return -1;
      }
    ...
    if (aafi->ctx.current_opgroup_affect_track) {
      ...
      if (!aafi->ctx.current_track) {
        TRACE_OBJ_ERROR(aafi, VaryingValue, &__td,
                        "Current track not set, dropping this volume");
        aafi_freeAudioGain(Gain);
        return -1;
      }
      if (aafi->ctx.current_track->gain) {
        TRACE_OBJ_ERROR(aafi, VaryingValue, &__td,
                        "Track volume was already set");
        aafi_freeAudioGain(Gain);
        return -1;
      } else {
        aafi->ctx.current_track->gain = Gain;
...
static int parse_EssenceDescriptor(AAF_Iface *aafi, aafObject *EssenceDesc,
  ...
  if (aafUIDCmp(EssenceDesc->Class->ID, &AAFClassID_PCMDescriptor)) {
    ...
    /* Compressed Audio (MP3, AAC ?). Not encountered yet (Davinci Resolve
     * describes MP3 using PCMDescriptor...) */
  ...
  aafObject *Locators =
      aaf_get_propertyValue(EssenceDesc, PID_EssenceDescriptor_Locator,
                            &AAFTypeID_LocatorStrongReferenceVector);
...
static int parse_PCMDescriptor(AAF_Iface *aafi, aafObject *PCMDescriptor,
  ...
  if (!audioEssenceFile) {
    TRACE_OBJ_ERROR(aafi, PCMDescriptor, &__td,
                    "aafi->ctx.current_audio_essence not set");
    return -1;
  }
  ...
  if (!channels) {
    TRACE_OBJ_ERROR(aafi, PCMDescriptor, &__td,
                    "Missing SoundDescriptor::Channels");
    return -1;
  }

  if (*channels >= USHRT_MAX) {
    TRACE_OBJ_ERROR(aafi, PCMDescriptor, &__td,
                    "SoundDescriptor::Channels bigger than USHRT_MAX");
    return -1;
  }
  ...
  if (!samplerate) {
    TRACE_OBJ_ERROR(aafi, PCMDescriptor, &__td,
                    "Missing FileDescriptor::SampleRate");
    return -1;
  }

  if (samplerate->denominator != 1) {
    TRACE_OBJ_ERROR(aafi, PCMDescriptor, &__td,
                    "FileDescriptor::SampleRate should be integer but is %i/%i",
  ...
  if (samplerate->numerator < 0) {
    TRACE_OBJ_ERROR(aafi, PCMDescriptor, &__td,
                    "FileDescriptor::SampleRate value is invalid : %i",
  ...
  uint32_t *samplesize = aaf_get_propertyValue(
      PCMDescriptor, PID_SoundDescriptor_QuantizationBits, &AAFTypeID_UInt32);

  if (!samplesize) {
    TRACE_OBJ_ERROR(aafi, PCMDescriptor, &__td,
                    "Missing SoundDescriptor::QuantizationBits");
    return -1;
  }
  ...
  if (*samplesize >= USHRT_MAX) {
    TRACE_OBJ_ERROR(
        ...
        "SoundDescriptor::QuantizationBits bigger than USHRT_MAX : %u",
        samplesize);
    return -1;
  }
...
static int parse_WAVEDescriptor(AAF_Iface *aafi, aafObject *WAVEDescriptor,
  ...
  if (!audioEssenceFile) {
    TRACE_OBJ_ERROR(aafi, WAVEDescriptor, &__td,
                    "aafi->ctx.current_audio_essence not set");
    return -1;
  }
  ...
  if (!summary) {
    TRACE_OBJ_ERROR(aafi, WAVEDescriptor, &__td,
                    "Missing WAVEDescriptor::Summary");
    return -1;
  }
...
static int parse_AIFCDescriptor(AAF_Iface *aafi, aafObject *AIFCDescriptor,
  ...
  if (!audioEssenceFile) {
    TRACE_OBJ_ERROR(aafi, AIFCDescriptor, &__td,
                    "aafi->ctx.current_audio_essence not set");
    return -1;
  }
  ...
  if (!summary) {
    TRACE_OBJ_ERROR(aafi, AIFCDescriptor, &__td,
                    "Missing AIFCDescriptor::Summary");
    return -1;
  }
...
static int parse_DigitalImageDescriptor(AAF_Iface *aafi,
  ...
  if (!videoEssenceFile) {
    TRACE_OBJ_ERROR(aafi, DIDescriptor, &__td,
  ...
  if (!framerate) {
    TRACE_OBJ_ERROR(aafi, DIDescriptor, &__td,
                    "Missing FileDescriptor::SampleRate (framerate)");
    return -1;
  }
...
static int parse_Locator(AAF_Iface *aafi, aafObject *Locator, td *__ptd) {
  ...
  if (aafUIDCmp(Locator->Class->ID, &AAFClassID_NetworkLocator)) {
    ...
    char *name =
        aaf_get_propertyValue(Locator, PID_TextLocator_Name, &AAFTypeID_String);
...
static int parse_NetworkLocator(AAF_Iface *aafi, aafObject *NetworkLocator,
  ...
  char *original_file_path = aaf_get_propertyValue(
      NetworkLocator, PID_NetworkLocator_URLString, &AAFTypeID_String);

  if (original_file_path == NULL) {
    TRACE_OBJ_ERROR(aafi, NetworkLocator, &__td,
                    "Missing NetworkLocator::URLString");
    goto err;
  }
...
static int parse_EssenceData(AAF_Iface *aafi, aafObject *EssenceData,
  ...
  if (!audioEssenceFile) {
    TRACE_OBJ_ERROR(aafi, EssenceData, &__td,
                    "aafi->ctx.current_audio_essence not set");
    return -1;
  }

  streamName = aaf_get_propertyValue(EssenceData, PID_EssenceData_Data,
                                     &AAFTypeID_String);
  ...
  if (!path) {
    TRACE_OBJ_ERROR(aafi, EssenceData, &__td,
                    "Could not retrieve EssenceData node path");
    goto err;
  }
  ...
  if (!dataPath) {
    TRACE_OBJ_ERROR(aafi, EssenceData, &__td,
                    "Could not build Data stream path");
    goto err;
  }
  ...
  if (!DataNode) {
    TRACE_OBJ_ERROR(aafi, EssenceData, &__td,
                    "Could not retrieve Data stream node: %s", dataPath);
    goto err;
  }
...
static int retrieve_UserComments(AAF_Iface *aafi, aafObject *UserComments,
  ...
  AAF_foreach_ObjectInSet(&UserComment, UserComments, NULL) {
    ...
    if (!aafUIDCmp(UserComment->Class->ID, &AAFClassID_TaggedValue)) {
      warning("Parsing UserComments: Expected TaggedValue but got %s",
              aaft_ClassIDToText(aafi->aafd, UserComment->Class->ID));
      goto UserCommentError;
    }

    name = aaf_get_propertyValue(UserComment, PID_TaggedValue_Name,
                                 &AAFTypeID_String);
    ...
    if (!text) {
      warning("Parsing UserComments: Could not retrieve Indirect value for "
              "TaggedValue::Value");
      goto UserCommentError;
    }
...
static int retrieve_ControlPoints(AAF_Iface *aafi, aafObject *Points,
   ...
   * ControlPoints to print, plus retrieve_ControlPoints() is being called
   * before VaryingValue Object is logged.
   */
  ...
  *values = calloc(Points->Header->_entryCount, sizeof(aafRational_t));
  ...
  AAF_foreach_ObjectInSet(&Point, Points, NULL) {
    ...
    if (!aafUIDCmp(Point->Class->ID, &AAFClassID_ControlPoint)) {
      error("Object is not AAFClassID_ControlPoint : %s",
            aaft_ClassIDToText(aafi->aafd, Point->Class->ID));
      continue;
    }
  ...
  if (Points->Header->_entryCount != i) {
    warning("ControlPoints _entryCount (%i) does not match iteration (%i).",
...
int aafi_retrieveData(AAF_Iface *aafi) {
  /* this __td is only here for debug/error, normal trace is printed from
   * parse_Mob() */
  static td __td;
  __td.fn = __LINE__;
  __td.pfn = 0;
  ...
  AAFI_foreach_ObjectInSet(&Mob, aafi->aafd->Mobs, i, __td) {
    ...
    if (aafUIDCmp(Mob->Class->ID, &AAFClassID_MasterMob) ||
      ...
      // TRACE_OBJ_WARNING( aafi, Mob, &__td, " PRINTS FOR DEBUG ONLY: Will be
      // parsed later" );
      continue;
    }

    if (!aafUIDCmp(Mob->Class->ID, &AAFClassID_CompositionMob)) {
      /* there should not be anything other than MasterMob, SourceMob or
       * CompositionMob */
      TRACE_OBJ_NO_SUPPORT(aafi, Mob, &__td);
      continue;
    }
    ...
    if (compositionMobParsed) {
      TRACE_OBJ_ERROR(aafi, Mob, &__td,
                      "Multiple top level CompositionMob not supported yet");
      continue;
    }
  ...
  AAFI_foreachAudioEssenceFile(aafi, audioEssenceFile) {
    ...
    if (!audioEssenceFile->is_embedded) {
      ...
      audioEssenceFile->usable_file_path = aafi_locate_external_essence_file(
          aafi, audioEssenceFile->original_file_path,
          aafi->ctx.options.media_location);

      if (audioEssenceFile->usable_file_path == NULL) {
        warning("Could not locate external audio essence file '%s'",
                audioEssenceFile->original_file_path);
      }
    }
  ...
  /*
   * Define AAF samplerate and samplesize with the most used values accross all
   * audio essences.
   */
  ...
  AAFI_foreachAudioEssenceFile(aafi, audioEssenceFile) {
    ...
    AAFI_foreachEssence(audioEssenceFile->next, ae) {
      if (audioEssenceFile->samplerate == ae->samplerate &&
          audioEssenceFile->samplesize == ae->samplesize) {
        count++;
      }
    }

    debug("Essence count @ %u Hz / %u bits : %i", audioEssenceFile->samplerate,
          audioEssenceFile->samplesize, count);
  ...
  AAFI_foreachVideoEssence(aafi, videoEssenceFile) {
    ...
    videoEssenceFile->usable_file_path = aafi_locate_external_essence_file(
        aafi, videoEssenceFile->original_file_path,
        aafi->ctx.options.media_location);

    if (videoEssenceFile->usable_file_path == NULL) {
      error("Could not locate external video essence file '%s'",
            videoEssenceFile->original_file_path);
      continue;
    }
  }
  ...
  AAFI_foreachAudioTrack(aafi, audioTrack) {
    ...
    if (aafi->compositionLength_editRate) {
      trackEnd =
          aafi_convertUnit(audioTrack->current_pos, audioTrack->edit_rate,
    ...
    if (trackEnd > aafi->compositionLength) {
      debug("Setting compositionLength with audio track \"%s\" (%u) : %" PRIi64,
            audioTrack->name, audioTrack->number, audioTrack->current_pos);
    ...
    AAFI_foreachTrackItem(audioTrack, audioItem) {
      ...
      audioClip->channels = aafi_getAudioEssencePointerChannelCount(
  ...
  AAFI_foreachVideoTrack(aafi, videoTrack) {
    ...
    if (aafi->compositionLength_editRate) {
      trackEnd =
          aafi_convertUnit(videoTrack->current_pos, videoTrack->edit_rate,
                           aafi->compositionLength_editRate);
    } else {
    ...
    if (trackEnd > aafi->compositionLength) {
      debug("Setting compositionLength with video track \"%s\" (%u) : %" PRIi64,
            videoTrack->name, videoTrack->number, videoTrack->current_pos);
...
void aafi_dump_obj(AAF_Iface *aafi, aafObject *Obj, struct trace_dump *__td,
                   int state, const char *func, int line, const char *fmt,
                   ...) {
  va_list args;
  ...
  if (aafi->ctx.options.trace == 0) {
    ...
    laaf_write_log(aafi->log, aafi, DEBUG_SRC_ID_AAF_IFACE, VERB_ERROR,
                   __FILENAME__, func, line, buf);
  ...
  if (Obj) {
    ...
    switch (state) {
    case TD_ERROR:
      LOG_BUFFER_WRITE(log, "%serr %s%s %s", ANSI_COLOR_RED(log),
                       ANSI_COLOR_DARKGREY(log), TREE_LINE,
                       ANSI_COLOR_RED(log));
      break;
    case TD_WARNING:
      LOG_BUFFER_WRITE(log, "%swrn %s%s %s", ANSI_COLOR_YELLOW(log),
                       ANSI_COLOR_DARKGREY(log), TREE_LINE,
                       ANSI_COLOR_YELLOW(log));
      break;
    case TD_NOT_SUPPORTED:
      LOG_BUFFER_WRITE(log, "%suns %s%s %s", ANSI_COLOR_ORANGE(log),
                       ANSI_COLOR_DARKGREY(log), TREE_LINE,
                       ANSI_COLOR_ORANGE(log));
      break;
    default:
    ...
    LOG_BUFFER_WRITE(log, "    %s%s%s      ", ANSI_COLOR_DARKGREY(log),
                     TREE_LINE, ANSI_COLOR_RESET(log));
  }

  LOG_BUFFER_WRITE(log, "%s%s%s", ANSI_COLOR_DARKGREY(log), TREE_LINE,
  ...
  if (Obj) {
    ...
    if (aaf_ObjectInheritsClass(Obj, &AAFClassID_Mob)) {
      ...
      aafMobID_t *mobID =
          aaf_get_propertyValue(Obj, PID_Mob_MobID, &AAFTypeID_MobIDType);
      ...
      LOG_BUFFER_WRITE(
          log, "(UsageCode: %s%s%s) %s%s", ANSI_COLOR_DARKGREY(log),
          aaft_UsageCodeToText(usageCode), ANSI_COLOR_RESET(log),
          (name && name[0] != 0x00) ? ": " : "", (name) ? name : "");

      free(name);

      LOG_BUFFER_WRITE(log, " MobID: %s%s%s", ANSI_COLOR_DARKGREY(log),
      ...
      LOG_BUFFER_WRITE(
          log, "[slot:%s%i%s track:%s%i%s] (DataDef: %s%s%s) %s%s ",
          ANSI_COLOR_BOLD(log), (slotID) ? (int)(*slotID) : -1,
          ANSI_COLOR_RESET(log), ANSI_COLOR_BOLD(log),
          (trackNo) ? (int)(*trackNo) : -1, ANSI_COLOR_RESET(log),
          (state == TD_NOT_SUPPORTED) ? ANSI_COLOR_ORANGE(log)
                                      : ANSI_COLOR_DARKGREY(log),
          aaft_DataDefToText(aafi->aafd, DataDefinition), ANSI_COLOR_RESET(log),
          (name && name[0] != 0x00) ? ": " : "", (name) ? name : "");
      ...
      aafWeakRef_t *OperationDefWeakRef =
          aaf_get_propertyValue(Obj, PID_OperationGroup_Operation,
                                &AAFTypeID_OperationDefinitionWeakReference);

      if (OperationDefWeakRef) {
        OperationIdentification = aaf_get_OperationIdentificationByWeakRef(
            aafi->aafd, OperationDefWeakRef);
      }

      int64_t *length = aaf_get_propertyValue(Obj, PID_Component_Length,
                                              &AAFTypeID_LengthType);

      LOG_BUFFER_WRITE(
          log, "(OpIdent: %s%s%s; Length: %s%li%s) ",
          (state == TD_NOT_SUPPORTED) ? ANSI_COLOR_ORANGE(log)
                                      : ANSI_COLOR_DARKGREY(log),
          aaft_OperationDefToText(aafi->aafd, OperationIdentification),
          ANSI_COLOR_RESET(log), ANSI_COLOR_DARKGREY(log),
          (length) ? *length : -1, ANSI_COLOR_RESET(log));
    } else if (aaf_ObjectInheritsClass(Obj, &AAFClassID_Component)) {
      int64_t *length = aaf_get_propertyValue(Obj, PID_Component_Length,
                                              &AAFTypeID_LengthType);

      LOG_BUFFER_WRITE(log, "(Length: %s%li%s", ANSI_COLOR_DARKGREY(log),
                       (length) ? *length : -1, ANSI_COLOR_RESET(log));

      if (aafUIDCmp(Obj->Class->ID, &AAFClassID_Transition)) {
        aafPosition_t *cutPoint = aaf_get_propertyValue(
            Obj, PID_Transition_CutPoint, &AAFTypeID_PositionType);

        if (cutPoint) {
          LOG_BUFFER_WRITE(log, "; CutPoint: %s%li%s", ANSI_COLOR_DARKGREY(log),
                           *cutPoint, ANSI_COLOR_RESET(log));
        }
      }
      ...
      aafIndirect_t *Indirect = aaf_get_propertyValue(
          Obj, PID_ConstantValue_Value, &AAFTypeID_Indirect);

      if (Indirect) {

        aafUID_t *ParamDef = aaf_get_propertyValue(
            Obj, PID_Parameter_Definition, &AAFTypeID_AUID);
        ...
        if (aafUIDCmp(ParamDef, &AAFParameterDef_Amplitude) &&
          ...
          aafRational_t *value =
              aaf_get_indirectValue(aafi->aafd, Indirect, NULL);

          LOG_BUFFER_WRITE(log, " %s(%+05.1lf dB)%s", ANSI_COLOR_DARKGREY(log),
      ...
      aafUID_t *ParamDef =
          aaf_get_propertyValue(Obj, PID_Parameter_Definition, &AAFTypeID_AUID);
      aafUID_t *InterpolationIdentification = NULL;

      aafWeakRef_t *InterpolationDefWeakRef = aaf_get_propertyValue(
          Obj, PID_VaryingValue_Interpolation,
          &AAFTypeID_InterpolationDefinitionWeakReference);

      if (InterpolationDefWeakRef) {
        InterpolationIdentification =
            aaf_get_InterpolationIdentificationByWeakRef(
                aafi->aafd, InterpolationDefWeakRef);
      }
      ...
      char *url = aaf_get_propertyValue(Obj, PID_NetworkLocator_URLString,
                                        &AAFTypeID_String);

      if (url) {
        LOG_BUFFER_WRITE(log, "(URLString: %s%s%s)", ANSI_COLOR_DARKGREY(log),
                         url, ANSI_COLOR_RESET(log));
      ...
      char *streamName =
          aaf_get_propertyValue(Obj, PID_EssenceData_Data, &AAFTypeID_String);

      if (streamName) {
        LOG_BUFFER_WRITE(log, "(Data: %s%s%s)", ANSI_COLOR_DARKGREY(log),
                         streamName, ANSI_COLOR_RESET(log));
      ...
      aafWeakRef_t *ContainerDefWeakRef =
          aaf_get_propertyValue(Obj, PID_FileDescriptor_ContainerFormat,
                                &AAFTypeID_ClassDefinitionWeakReference);

      if (ContainerDefWeakRef) {
        ContainerFormat = aaf_get_ContainerIdentificationByWeakRef(
            aafi->aafd, ContainerDefWeakRef);
      }

      LOG_BUFFER_WRITE(
          log, "(ContainerIdent : %s%s%s)", ANSI_COLOR_DARKGREY(log),
          aaft_ContainerToText(ContainerFormat), ANSI_COLOR_RESET(log));
    }
    ...
    int rc = laaf_util_vsnprintf_realloc(&log->_msg, &log->_msg_size,
                                         log->_msg_pos, fmt, args);
    ...
    if (!aafi->ctx.options.dump_class_aaf_properties) {
      ...
      for (Prop = Obj->Properties; Prop != NULL; Prop = Prop->next) {
        if (Prop->def->meta) {
          LOG_BUFFER_WRITE(log, "%s%s %s[0x%04x]", ANSI_COLOR_RESET(log),
    ...
    if (aafi->ctx.options.dump_tagged_value) {
      if (aaf_ObjectInheritsClass(Obj, &AAFClassID_Mob)) {
        ...
        aafObject *UserComments =
            aaf_get_propertyValue(Obj, PID_Mob_UserComments,
                                  &AAFTypeID_TaggedValueStrongReferenceVector);
        aafObject *Attributes =
            aaf_get_propertyValue(Obj, PID_Mob_Attributes,
                                  &AAFTypeID_TaggedValueStrongReferenceVector);
        ...
        aafObject *UserComments =
            aaf_get_propertyValue(Obj, PID_Component_UserComments,
                                  &AAFTypeID_TaggedValueStrongReferenceVector);
        aafObject *Attributes =
            aaf_get_propertyValue(Obj, PID_Component_Attributes,
                                  &AAFTypeID_TaggedValueStrongReferenceVector);
          ...
          if (aafi->ctx.options.dump_meta) {
            ...
            if (Prop->sf == SF_STRONG_OBJECT_REFERENCE_VECTOR) {
              ...
              LOG_BUFFER_WRITE(log, "    [%s0x%04x%s] %s (%s)\n",
                               ANSI_COLOR_MAGENTA(log), Prop->pid,
                               ANSI_COLOR_RESET(log),
                               aaft_PIDToText(aafi->aafd, Prop->pid),
                               aaft_StoredFormToText(Prop->sf));

              void *propValue = aaf_get_propertyValue(
                  Obj, Prop->pid, &AAFTypeID_TaggedValueStrongReferenceVector);
    ...
    if (aafi->ctx.options.dump_class_raw_properties &&
        strcmp(aaft_ClassIDToText(aafi->aafd, Obj->Class->ID),
      ...
      aaf_dump_nodeStreamProperties(
          aafi->aafd,
          cfb_getChildNode(aafi->aafd->cfbd, "properties", Obj->Node), "    ");
  ...
  log->debug_callback(log, (void *)aafi, DEBUG_SRC_ID_TRACE, 0, "", "", 0,
                      log->_msg, log->user);
