...
(MetaProps: DataOffset[0xffde])
        ...
        // 	aafi_applyGainOffset( aafi, &aafi->ctx.current_clip->gain,
...
void aafi_dump_obj(AAF_Iface *aafi, aafObject *Obj, struct trace_dump *__td,
  ...
  if (aafi->ctx.options.trace == 0) {
    ...
    laaf_write_log(aafi->log, aafi, LOG_SRC_ID_AAF_IFACE, VERB_ERROR,
  ...
  log->log_callback(log, (void *)aafi, LOG_SRC_ID_TRACE, 0, "", "", 0,
                    log->_msg, log->user);
