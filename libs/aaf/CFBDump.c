...
void cfb_dump_node(CFB_Data *cfbd, cfbNode *node, int print_stream,
  ...
  LOG_BUFFER_WRITE(log, "%s_ab          : %s%s%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), nodeName, ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_cb          : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), node->_cb, ANSI_COLOR_RESET(log));
  ...
  LOG_BUFFER_WRITE(log, "%s_bflags      : %s%s%s\n", padding,
                   ANSI_COLOR_DARKGREY(log),
                   node->_bflags == 1 ? "BLACK" : "RED", ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_sidLeftSib  : %s0x%08x%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), node->_sidLeftSib,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_sidRightSib : %s0x%08x%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), node->_sidRightSib,
                   ANSI_COLOR_RESET(log));

  if (node->_mse == STGTY_STORAGE || node->_mse == STGTY_ROOT) {
    LOG_BUFFER_WRITE(log, "%s_sidChild    : %s0x%08x%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), node->_sidChild,
                     ANSI_COLOR_RESET(log));
    LOG_BUFFER_WRITE(log, "%s_clsid       : %s%s%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), cfb_CLSIDToText(&(node->_clsId)),
                     ANSI_COLOR_RESET(log));
    LOG_BUFFER_WRITE(log, "%s_dwUserFlags : %s0x%08x (%d)%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), node->_dwUserFlags,
                     node->_dwUserFlags, ANSI_COLOR_RESET(log));
  }

  if (node->_mse == STGTY_INVALID) {
    LOG_BUFFER_WRITE(log, "%s_time  (cre) : %s0x%08x%08x%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), node->_time[0].dwHighDateTime,
                     node->_time[0].dwLowDateTime, ANSI_COLOR_RESET(log));

    LOG_BUFFER_WRITE(log, "%s_      (mod) : %s0x%08x%08x%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), node->_time[1].dwHighDateTime,
                     node->_time[1].dwLowDateTime, ANSI_COLOR_RESET(log));
  }

  if (node->_mse == STGTY_STREAM || node->_mse == STGTY_ROOT) {
    LOG_BUFFER_WRITE(log, "%s_sectStart   : %s0x%08x (%d)%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), node->_sectStart,
                     node->_sectStart, ANSI_COLOR_RESET(log));
    LOG_BUFFER_WRITE(log, "%s_ulSizeLow   : %s0x%08x (%d)%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), node->_ulSizeLow,
                     node->_ulSizeLow, ANSI_COLOR_RESET(log));
    LOG_BUFFER_WRITE(log, "%s_ulSizeHigh  : %s0x%08x (%d)%s\n", padding,
                     ANSI_COLOR_DARKGREY(log), node->_ulSizeHigh,
                     node->_ulSizeHigh, ANSI_COLOR_RESET(log));
  }

  LOG_BUFFER_WRITE(log, "\n\n");

  log->log_callback(log, (void *)cfbd, LOG_SRC_ID_DUMP, 0, "", "", 0, log->_msg,
                    log->user);
...
void cfb_dump_nodeStream(CFB_Data *cfbd, cfbNode *node, const char *padding) {
  ...
  laaf_util_dump_hex(stream, stream_sz, &log->_msg, &log->_msg_size,
                     log->_msg_pos, padding);
...
void cfb_dump_nodePaths(CFB_Data *cfbd, uint32_t prevPath, char *strArray[],
  ...
  laaf_util_snprintf_realloc(&strArray[thisPath], 0, 0, "%s/%s",
                             strArray[prevPath], nodeName);
  ...
  if ((int32_t)node->_sidChild > 0)
    cfb_dump_nodePaths(cfbd, thisPath, strArray, str_i,
                       &cfbd->nodes[node->_sidChild], padding, 0);

  if ((int32_t)node->_sidLeftSib > 0)
    cfb_dump_nodePaths(cfbd, prevPath, strArray, str_i,
                       &cfbd->nodes[node->_sidLeftSib], padding, 0);

  if ((int32_t)node->_sidRightSib > 0)
    cfb_dump_nodePaths(cfbd, prevPath, strArray, str_i,
                       &cfbd->nodes[node->_sidRightSib], padding, 0);

  /* the end of the first function call, recursion is over. */
  if (firstIteration) {

    /* commented out because output seems proper this way... why did we call
    ...
    for (uint32_t i = 0; i < cfbd->nodes_cnt && strArray[i] != NULL; i++) {
      LOG_BUFFER_WRITE(log, "%s%0*i : %s%s%s\n", padding,
                       (cfbd->nodes_cnt > 1000000)  ? 7
                       : (cfbd->nodes_cnt > 100000) ? 6
                       : (cfbd->nodes_cnt > 10000)  ? 5
                       : (cfbd->nodes_cnt > 1000)   ? 4
                       : (cfbd->nodes_cnt > 100)    ? 3
                       : (cfbd->nodes_cnt > 10)     ? 2
                                                    : 1,
                       i, ANSI_COLOR_DARKGREY(log), strArray[i],
                       ANSI_COLOR_RESET(log));
      free(strArray[i]);
    }
    ...
    log->log_callback(log, (void *)cfbd, LOG_SRC_ID_DUMP, 0, "", "", 0,
                      log->_msg, log->user);
  }
}
...
void cfb_dump_header(CFB_Data *cfbd, const char *padding) {
  ...
  LOG_BUFFER_WRITE(log, "%s_abSig              : %s0x%08" PRIx64 "%s\n",
                   padding, ANSI_COLOR_DARKGREY(log), cfbh->_abSig,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_clsId              : %s%s%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfb_CLSIDToText(&(cfbh->_clsid)),
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_version            : %s%u.%u ( 0x%04x 0x%04x )%s\n",
                   padding, ANSI_COLOR_DARKGREY(log), cfbh->_uMinorVersion,
                   cfbh->_uDllVersion, cfbh->_uMinorVersion, cfbh->_uDllVersion,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_uByteOrder         : %s%s ( 0x%04x )%s\n", padding,
                   ...
                   : cfbh->_uByteOrder == 0xFEFF ? "big-endian"
                                                 : "?",
                   cfbh->_uByteOrder, ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_uSectorShift       : %s%u (%u bytes sectors)%s\n",
                   padding, ANSI_COLOR_DARKGREY(log), cfbh->_uSectorShift,
                   1 << cfbh->_uSectorShift, ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log,
                   "%s_uMiniSectorShift   : %s%u (%u bytes mini-sectors)%s\n",
                   padding, ANSI_COLOR_DARKGREY(log), cfbh->_uMiniSectorShift,
                   1 << cfbh->_uMiniSectorShift, ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_usReserved0        : %s0x%02x%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_usReserved,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_ulReserved1        : %s0x%04x%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_ulReserved1,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_csectDir           : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_csectDir,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_csectFat           : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_csectFat,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_sectDirStart       : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_sectDirStart,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_signature          : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_signature,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_ulMiniSectorCutoff : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_ulMiniSectorCutoff,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_sectMiniFatStart   : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_sectMiniFatStart,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_csectMiniFat       : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_csectMiniFat,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_sectDifStart       : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_sectDifStart,
                   ANSI_COLOR_RESET(log));
  LOG_BUFFER_WRITE(log, "%s_csectDif           : %s%u%s\n", padding,
                   ANSI_COLOR_DARKGREY(log), cfbh->_csectDif,
                   ANSI_COLOR_RESET(log));
...
void cfb_dump_FAT(CFB_Data *cfbd, const char *padding) {
  ...
  LOG_BUFFER_WRITE(log, "_CFB_FAT______________________________________________"
  ...
  for (i = 0; i < cfbd->fat_sz; i++) {
    LOG_BUFFER_WRITE(log, "%sSECT[%s%0*u%s] : %s0x%08x %s%s\n", padding,
                     ANSI_COLOR_DARKGREY(log),
                     (cfbd->fat_sz > 1000000)  ? 7
                     : (cfbd->fat_sz > 100000) ? 6
                     : (cfbd->fat_sz > 10000)  ? 5
                     : (cfbd->fat_sz > 1000)   ? 4
                     : (cfbd->fat_sz > 100)    ? 3
                     : (cfbd->fat_sz > 10)     ? 2
                                               : 1,
                     i, ANSI_COLOR_RESET(log),

                     ANSI_COLOR_DARKGREY(log), cfbd->fat[i],
...
void cfb_dump_MiniFAT(CFB_Data *cfbd, const char *padding) {
  ...
  LOG_BUFFER_WRITE(log, "_CFB_MiniFAT__________________________________________"
                        "_________________________________________\n\n");
  ...
  for (i = 0; i < cfbd->miniFat_sz; i++) {
    LOG_BUFFER_WRITE(
        log, "%sSECT[%s%0*u%s] : %s0x%08x %s%s\n", padding,
        ANSI_COLOR_DARKGREY(log),
        (cfbd->miniFat_sz > 1000000)  ? 7
        : (cfbd->miniFat_sz > 100000) ? 6
        ...
        ANSI_COLOR_DARKGREY(log), cfbd->miniFat[i],
...
void cfb_dump_DiFAT(CFB_Data *cfbd, const char *padding) {
  ...
  LOG_BUFFER_WRITE(log, "_CFB_DiFAT____________________________________________"
                        "_________________________________________\n\n");
  ...
  for (i = 0; i < cfbd->DiFAT_sz; i++) {
    LOG_BUFFER_WRITE(log, "%sSECT[%s%0*u%s] : %s0x%08x %s%s\n", padding,
                     ...
                     ANSI_COLOR_DARKGREY(log), cfbd->DiFAT[i],
