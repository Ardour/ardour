...
static uint64_t cfb_readFile(CFB_Data *cfbd, unsigned char *buf, size_t offset,
                             size_t len);
...
const char *cfb_CLSIDToText(const cfbCLSID_t *clsid) {
  ...
  if (clsid == NULL) {
    ...
    int rc = snprintf(str, sizeof(str),
...
int cfb_load_file(CFB_Data **cfbd_p, const char *file) {
  ...
  // debug( "MiniFAT sector (%u) per FAT setor (%u): %u", (1 <<
  // cfbd->hdr->_uMiniSectorShift), (1 << cfbd->hdr->_uSectorShift), (1 <<
  // cfbd->hdr->_uSectorShift) / (1 << cfbd->hdr->_uMiniSectorShift) );
...
static uint64_t cfb_readFile(CFB_Data *cfbd, unsigned char *buf, size_t offset,
  ...
  // debug( "Requesting file read @ offset %"PRIu64" of length %"PRIu64, offset,
  // reqlen );
  ...
  if (reqlen + offset > cfbd->file_sz) {
    error("Requested data goes %" PRIu64
          ...
          (reqlen + offset) - cfbd->file_sz, offset, reqlen);
    return 0;
  }
  ...
  if (feof(fp)) {
    if (byteRead < reqlen) {
      error("Incomplete fread() of CFB due to EOF : %" PRIu64
    ...
    if (byteRead < reqlen) {
      error("Incomplete fread() of CFB due to error : %" PRIu64
            " bytes read out of %" PRIu64 " requested",
            byteRead, reqlen);
    } else {
      error("fread() error of CFB : %" PRIu64 " bytes read out of %" PRIu64
            " requested",
            byteRead, reqlen);
    }
  }
...
unsigned char *cfb_getSector(CFB_Data *cfbd, cfbSectorID_t id) {
  ...
  // laaf_util_dump_hex( buf, (1<<cfbd->hdr->_uSectorShift), &cfbd->log->_msg,
  // &cfbd->log->_msg_size, cfbd->log->_msg_pos, "" );
...
unsigned char *cfb_getMiniSector(CFB_Data *cfbd, cfbSectorID_t id) {
  ...
  for (i = 0; i < id / fatDiv; i++) {
    ...
    if (cfbd->fat[fatId] >= CFB_MAX_REG_SID) {
      error("Next FAT index (%i/%i) is invalid: %u (%08x)", i, (id / fatDiv),
            cfbd->fat[fatId], cfbd->fat[fatId]);
      goto err;
    }

    if (cfbd->fat[fatId] >= cfbd->fat_sz) {
      error("Next FAT index (%i/%i) is bigger than FAT size (%u): %u (%08x)", i,
            (id / fatDiv), cfbd->fat_sz, cfbd->fat[fatId], cfbd->fat[fatId]);
      goto err;
    }
...
static int cfb_retrieveFileHeader(CFB_Data *cfbd) {
  ...
  if (cfbd->hdr->_uSectorShift != 9 && cfbd->hdr->_uSectorShift != 12) {
    goto err;
  }
...
static int cfb_retrieveDiFAT(CFB_Data *cfbd) {
  ...
  if (cfbd->hdr->_csectFat > 109) {
    ...
    double csectDifdouble = ceil((float)((cfbd->hdr->_csectFat - 109) * 4) /
  ...
  if (csectDif != cfbd->hdr->_csectDif) {
    warning("cfbd->hdr->_csectDif value (%u) does not match calculated "
            "csectDif (%u)",
   ...
   * _uSectorShift is guaranted to be 9 or 12, so DiFAT_sz will never override
   * UINT_MAX
   */
  ...
  /* _uSectorShift is guaranted to be 9 or 12, so sectorSize will never be
   * negative */
...
static int cfb_retrieveMiniFAT(CFB_Data *cfbd) {
  ...
  uint32_t miniFat_sz = cfbd->hdr->_csectMiniFat *
...
static int cfb_retrieveNodes(CFB_Data *cfbd) {
  ...
  if (cfbd->hdr->_uSectorShift == 9) { /* 512 bytes sectors */
    ...
    /* _uSectorShift is guaranted to be 9 or 12, so nodesPerSect will never
     * override UINT_MAX */
    uint32_t nodesPerSect =
        (1U << cfbd->hdr->_uMiniSectorShift) / sizeof(cfbNode);
  ...
  while (1) {
    ...
    if (rc == 0) {
      ...
      if (pathLen == nameLen)
        ...
        return cfb_getNodeByPath(cfbd, path + nameLen,
                                 cfbd->nodes[id]._sidChild);
    } else if (rc > 0)
...
cfbNode *cfb_getChildNode(CFB_Data *cfbd, const char *name,
                          cfbNode *startNode) {
  int rc = 0;
