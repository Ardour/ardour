...
int laaf_riff_writeWavFileHeader(FILE *fp, struct wavFmtChunk *wavFmt,
                                 struct wavBextChunk *wavBext,
                                 uint32_t audioDataSize, struct aafLog *log) {

  (void)log;
  size_t filesize = (4 /* WAVE */) + sizeof(struct wavFmtChunk) +
...
int laaf_riff_parseAudioFile(struct RIFFAudioFile *RIFFAudioFile,
  ...
  if (bytesRead == RIFF_READER_ERROR || bytesRead < sizeof(riff)) {
  ...
  while (pos < filesize) {
    ...
    if (bytesRead == RIFF_READER_ERROR || bytesRead < sizeof(chunk)) {
      error("Could not read chunk \"%.4s\" @ %" PRIu64 " (%" PRIu64
            " bytes returned)",
            chunk.ckid, pos, bytesRead);
      break;
    }
    ...
    debug("Got chunk \"%.4s\" (%u bytes) @ %" PRIu64 " (%" PRIu64
          " bytes returned)",
          chunk.ckid, chunk.cksz, pos, bytesRead);
    ...
    if (!be) { /* WAVE */
      ...
      if (chunk.ckid[0] == 'f' && chunk.ckid[1] == 'm' &&
        ...
        if (bytesRead == RIFF_READER_ERROR || bytesRead < sizeof(riff)) {
          error("Could not read chunk \"%.4s\" content @ %" PRIu64 " (%" PRIu64
                " bytes returned)",
                chunk.ckid, pos, bytesRead);
          break;
        }
        ...
        if (RIFFAudioFile->channels > 0 && RIFFAudioFile->sampleSize > 0) {
          RIFFAudioFile->sampleCount = chunk.cksz / RIFFAudioFile->channels /
                                       (RIFFAudioFile->sampleSize / 8);
        }

        RIFFAudioFile->pcm_audio_start_offset =
            (pos + sizeof(struct riffChunk));
      ...
      } else if (chunk.ckid[0] == 'S' && chunk.ckid[1] == 'S' &&
                 chunk.ckid[2] == 'N' && chunk.ckid[3] == 'D') {
        /*
         * Samplecount should be already set with numSampleFrames in COMM chunk.
         * However in AAF (AIFCDescriptor::Summary), numSampleFrames is often
         * null, so we must extract samplecount out of SSND chunk, like we do
         * with wav DATA chunk.
         */
        uint64_t sampleCount = chunk.cksz / RIFFAudioFile->channels /
                               (RIFFAudioFile->sampleSize / 8);

        if (RIFFAudioFile->sampleCount > 0 &&
            RIFFAudioFile->sampleCount != sampleCount) {
          debug("Sample count retrieved from COMM chunk (%" PRIu64
                ") does not match SSND chunk (%" PRIu64 ")",
                RIFFAudioFile->sampleCount, sampleCount);
        }

        RIFFAudioFile->sampleCount = sampleCount;
        RIFFAudioFile->pcm_audio_start_offset =
            pos + sizeof(struct aiffSSNDChunk);
