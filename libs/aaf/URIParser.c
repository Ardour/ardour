...
static char *uriDecodeString(char *src, char *dst);
static int uriIsIPv4(const char *s, size_t size, char **err);
static int uriIsIPv6(const char *s, size_t size, char **err);
...
static int _uri_parse_authority(struct uri *uri, const char **pos,
                                const char *end, struct aafLog *log);
...
struct uri *laaf_uri_parse(const char *uristr, enum uri_option optflags,
                           struct aafLog *log) {
...
void laaf_uri_free(struct uri *uri) {
  if (!uri) {
    return;
  }
...
static int uriIsIPv4(const char *s, size_t size, char **err) {
  ...
  if (octets > 4) {
    if (err) {
      laaf_util_snprintf_realloc(err, NULL, 0,
...
static int uriIsIPv6(const char *s, size_t size, char **err) {
  ...
  for (size_t i = 0; i <= size; i++) {
    ...
    if (prev == 'e' || prev == 'c') { /* empty or colon */
      ...
      if (*(s + i) == ':') {
        if (err) {
          laaf_util_snprintf_realloc(err, NULL, 0,
                                     "can't have more than two successive ':'");
        }
        return 0;
      }
    }
    ...
    if (err) {
      laaf_util_snprintf_realloc(err, NULL, 0, "illegal char '%c' (0x%02x)",
                                 *(s + i), *(s + i));
    }
  ...
  if (ipv4portion > 4) {
    if (err) {
      laaf_util_snprintf_realloc(
          err, NULL, 0, "too many octets in ipv4 portion : %i", ipv4portion);
    }
    return 0;
  }
  if (ipv4portion > 0 && ipv4portion < 4) {
    if (err) {
      laaf_util_snprintf_realloc(
          err, NULL, 0, "not enough octets in ipv4 portion : %i", ipv4portion);
    }
    return 0;
  }
