 ...
 * @brief Retrieves the AAF Objects Tree from the Compound File Binary.
 *
 * The AAF file structure is based on the Compound File Binary, implemented by
 * LibCFB and defined at https://www.amwa.tv/projects/MS-03.shtml
 *
 * The specifications of the low-level AAF can be found at
 * https://amwa.tv/projects/MS-02.shtml
 *
 * The specifications of the overall AAF and standard Classes / Properties can
 * be found at https://amwa.tv/projects/MS-01.shtml
 *
 ...
 * - provide functions to navigate in the tree and access objects and
 * properties.
 *
 * Therefore, AAFCore makes a bridge between the low-level Compound File Binary
 * and the high level AAFIface, which is used to interpret objects and thus
 * retrieve usable data.
 *
 * Even though AAFCore can be used as is, it is recommended for complex
 * operations like essences and clips retrieval, to use AAFIface.
 *
 * ### Usage
 *
 * In order to use libAAF, you should start by allocating AAF_Data with
 * aaf_alloc(), then you can load a file by calling aaf_load_file().
 * aaf_load_file() will not load the entire file to memory, but will instead
 * parse the CFB Tree and set AAF_Data accordingly.
 *
 * @code
 ...
 * @endcode
 *
 * Then, you can access the object tree thanks to the access functions, or use
 * the AAFIface to extract usable data through a convenient interface.
 *
 * @code
 * // TODO
 * @endcode
 *
 * Once you're done, you can close the file and free the AAF_Data structure by
 * calling aaf_release( &aafd );
 *
 * @code
 * aaf_release( &aafd );
 * @endcode
...
typedef struct aafPropertyDefinition {
   ...
   * files had incorrect values for Type's Name and Identification, even Avid's
   * files. Thus, PDef->type should NOT be trusted.
   *
...
enum aafClassIsConcrete_e { ABSTRACT = 0, ABS = 0, CONCRETE = 1, CON = 1 };
...
typedef struct aafObject {
  ...
  /* keeps track of aafd, mostly to access aafd->verb without having to pass
   * aafd to some functions */
...
typedef struct _aafData {
   ...
   *
   * @note This list is intended to keep track of all the allocated Objects, not
   * for parsing. For tree access, the AAF_Data.Root pointer should be used.
   */
...
#define aafUIDCmp(auid1, auid2)                                                \
  ((auid1) != NULL && ((auid2)) != NULL && (auid1)->Data1 == (auid2)->Data1 && \
   (auid1)->Data2 == (auid2)->Data2 && (auid1)->Data3 == (auid2)->Data3 &&     \
...
#define aafSlotIDCmp(slotID1, slotID2) ((slotID1) == (slotID2))
...
/**
 * Releases the CFB_Data structure, fclose() the file and frees the AAF_Data
 * structure.
 *
 * @param  aafd  Pointer to the AAF_Data structure.
 */
...
/**
 * Resolves a given WeakReference from a given Dictionary (a Set or Vector list
 * of aafObjects), and returns a pointer to the corresponding Object if it was
 * found.
 *
 * @param ref  Pointer to the WeakReference to search for.
 * @param list Pointer to the aafObject list from which the Reference will be
 * searched for.
 *
...
/**
 * Loops through each aafObject of a list, that is of a Set or Vector. It is
 * also possible to filter the returned Object by ClassID.
 *
 * This function should be used as the conditional expression of a while loop.
 * The aaf_foreach_ObjectInSet() is an implementation of this.
 *
 * @param  Obj    Pointer to pointer to an aafObject structure that will receive
 * each Object of the list.
 * @param  head   Pointer to the first Object of the list.
 * @param  filter Pointer to a ClassID to use as a filter, shall be NULL if
 * unused.
 *
...
/**
 * Convenience macro, that implements the _aaf_foreach_ObjectInSet() function.
 * This should be used istead of directly calling _aaf_foreach_ObjectInSet().
 */
...
/**
 * Safely get an Indirect value, after it was retrieved using
 * aaf_get_propertyValue(). Function checks value type and in case of
 * AAFTypeID_String, performs allocation and UTF8 conversion.
 *
 ...
 *
 * @return            A pointer to the Indirect value, or a pointer to an
 * allocated char if Indirect is AAFTypeID_String\n NULL in case of error.
 */
