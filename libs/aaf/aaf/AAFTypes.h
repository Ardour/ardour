...
static const aafUID_t AAFUID_NULL = {
    0x00000000,
    0x0000,
    0x0000,
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}};
...
static const aafMobID_t AAFMOBID_NULL = {
...
typedef struct _aafIndirect_t {
  /*
   * byteOrder disabled for memory alignement and to avoid
   * -Waddress-of-packed-member It is always little-endian (0x4c), or
   * unspecified (0x55) but LE, anyway.
   */
...
typedef enum _aafElectroSpatialFormulation_e {
  ...
  AAFElectroSpatialFormulation_StereoRightChannelDoubleSamplingFrequencyMode =
      9,
 ...
 * structures, which are then followed by aafPropertyIndexHeader_t._entryCount
 * variable sized property values.
 */
 ...
 * @code
 *
 * int offset = sizeof(aafPropertyIndexHeader_t) +
 * (aafPropertyIndexHeader_t._entryCount * sizeof(aafPropertyIndexEntry_t))
 *
 * @endcode
 ...
 * An ordered collection of strongly referenced (contained) objects.
 * Each vector index consists of an aafStrongRefVectorHeader_t
 * followed by aafStrongRefVectorHeader_t._entryCount aafStrongRefVectorEntry_t
 * structs.
 */
