...
#define ANSI_COLOR_RESET(log)                                                  \
  (((log)->ansicolor) ? (log->color_reset) ? log->color_reset : "\x1b[0m" : "")
...
size_t laaf_util_utf8strCharLen(const char *u8str);

char *laaf_util_utf16Toutf8(const uint16_t *u16str);

#ifdef _WIN32
wchar_t *laaf_util_windows_utf8toutf16(const char *str);
char *laaf_util_windows_utf16toutf8(const wchar_t *wstr);
#endif
...
char *laaf_util_relative_path(const char *filepath, const char *refpath);

char *laaf_util_absolute_path(const char *relpath);

char *laaf_util_c99strdup(const char *src);

int laaf_util_snprintf_realloc(char **str, size_t *size, size_t offset,
                               const char *format, ...);

int laaf_util_vsnprintf_realloc(char **str, size_t *size, size_t offset,
                                const char *fmt, va_list args);

int laaf_util_dump_hex(const unsigned char *stream, size_t stream_sz,
                       char **buf, size_t *bufsz, size_t offset,
                       const char *padding);
