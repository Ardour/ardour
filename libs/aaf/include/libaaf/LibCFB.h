...
/**
 * A stream identifier, that is an index into the array of nodes (directory
 * entries).
 */
 ...
 *  NOTE: enum was turned to MACRO, because of -Wpedantic (and even though they
 * are valid 32bits...)
 */
 ...
 *
 *  NOTE: microsoft already define enum tagSTGTY, but it lacks of STGTY_INVALID
 * and STGTY_ROOT.
 */
...
typedef enum tagDECOLOR { CFB_RED = 0, CFB_BLACK = 1 } cfbColor_e;
...
/**
 * The length of the cfbNode._ab uint16_t array holding the node's UTF-16 name,
 * including the NULL terminating Unicode.
 */

#define CFB_NODE_NAME_SZ 32

/**
 * This is an arbitrary length chosen for defining a char array to store a path
...
/**
 * This is the header of the Compound File. It corresponds to the first 512
 * bytes of the file starting at offset zero. If the sector size is greater than
 * 512 bytes, then the header is padded to the sector size with zeroes.
 */
...
typedef struct StructuredStorageHeader {
  ...
  /**
   * CFB spec says this should be set to zero, however AAF gets the file kind
   * from this CLSID. Thus, this CLSID should match one of the AAFFileKind_*
   * from AAFDefs/AAFFileKinds.h
   */
  ...
  /**
   * Size of mini-sectors in power-of-two. Typical value is 6 (64-byte
   * mini-sectors)
   */
   ...
   *
   * These are the first entries in the DiFAT. If the _csectDif is zero, then
   * this is the entire DiFAT.
   */
...
/**
 * This structure represents a Node in the Directory stream and thus a node in
 * the Directory Tree. The Compound File Directory spec calls a "Node" a
 * "Directory".
 *
 * The Directory stream starts at sector ID cfbHeader._sectDirStart in the FAT
 * and ends at sector ID CFB_NO_STREAM. Each directory sector is an array of
 * directory entries (cfbNode), so the entire directory stream forms the array
 * of nodes. An index into that array is called a stream identifier (SID).
 *
 ...
 *
 * The Directory Tree forms a red/black Tree composed of each cfbNode, in which
 * the root node is the first cfbNode entry (SID 0).
 */
...
typedef struct StructuredStorageDirectoryEntry {
   ...
   *
   * A 64-byte array, for a maximum of 32 Unicode characters including a
   * terminating Unicode NULL character. The string shall be padded with zeros
   * to fill the array.
   *
   *  Should be wchar_t, but on linux wchar_t is 32 bits wide as opposed to
   * windows (and thus CFB) defining wchar_t to 16 bits. Conversion is done by
   * cfb_w16towchar()
   */
  ...
  /**
   * Length of the node's name in bytes, including the Unicode NULL terminating
   * byte.
   */
  ...
  /**
   * Array of two cfbFiletime_t struct. The first one holds the creation
   * date/time, the second the modification date/time.
   *
...
#define cfb_foreachSectorInChain(cfbd, buf, id)                                \
  for (buf = cfb_getSector(cfbd, id); id < CFB_MAX_REG_SECT && buf != NULL;    \
       id = cfbd->fat[id], buf = cfb_getSector(cfbd, id))
...
#define cfb_foreachMiniSectorInChain(cfbd, buf, id)                            \
  for (buf = cfb_getMiniSector(cfbd, id); id < CFB_MAX_REG_SECT;               \
       id = cfbd->miniFat[id], buf = cfb_getMiniSector(cfbd, id))

/**
 * Loops through each DiFAT sector.
 *
 * @param cfbd Pointer to the CFB_Data structure.
 * @param buf  Pointer to the buffer that will hold each DiFAT sector data
 * bytes.
 * @param id   Index of each DiFAT sector. The first index is retrieved from the
 * Compound File Binary Header, so user can pass any value (usualy 0). Then, the
 * ID of the next DiFAT sector is retrieved from the last 4 bytes of each DiFAT
 *             sector data.
 */

#define cfb_foreachSectorInDiFATChain(cfbd, buf, id)                           \
  for (id = cfbd->hdr->_sectDifStart, buf = cfb_getSector(cfbd, id);           \
       id < CFB_MAX_REG_SECT;                                                  \
       memcpy(&id, (buf + (1 << cfbd->hdr->_uSectorShift) - 4),                \
              sizeof(uint32_t)),                                               \
      free(buf), buf = cfb_getSector(cfbd, id))
...
#define cfb_getNodeStreamLen(cfbd, node)                                       \
  ((cfbd->hdr->_uSectorShift > 9)                                              \
       ? (uint64_t)(((uint64_t)(node->_ulSizeHigh) << 32) |                    \
                    (node->_ulSizeLow))                                        \
       : node->_ulSizeLow)
...
uint64_t cfb_getStream(CFB_Data *cfbd, cfbNode *node, unsigned char **stream,
                       uint64_t *stream_sz);

int cfb__foreachSectorInStream(CFB_Data *cfbd, cfbNode *node,
                               unsigned char **buf, size_t *bytesRead,
                               cfbSectorID_t *sectID);
...
cfbNode *cfb_getChildNode(CFB_Data *cfbd, const wchar_t *name,
                          cfbNode *startNode);
