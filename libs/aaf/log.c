...
void laaf_log_callback(struct aafLog *log, void *ctxdata, int libid, int type,
...
#ifdef __MINGW32__
    fwprintf(log->fp,
             L"[%" WPRIs "%" WPRIs "%" WPRIs "] %" WPRIs "%" WPRIs
             ":%i in %" WPRIs "()%" WPRIs " : ",
             color, typestr, ANSI_COLOR_RESET(log), ANSI_COLOR_DARKGREY(log),
             srcfile, lineno, srcfunc, ANSI_COLOR_RESET(log));
#else
    fprintf(log->fp, "[%s%s%s] %s%s:%i in %s()%s : ", color, typestr,
            ANSI_COLOR_RESET(log), ANSI_COLOR_DARKGREY(log), srcfile, lineno,
            srcfunc, ANSI_COLOR_RESET(log));
#endif
  }
...
void laaf_write_log(struct aafLog *log, void *ctxdata, enum debug_source_id lib,
                    enum verbosityLevel_e type, const char *dbgfile,
                    const char *dbgfunc, int dbgline, const char *format, ...) {
  if (!log) {
    return;
  }
  ...
  log->debug_callback(log, (void *)ctxdata, lib, type, dbgfile, dbgfunc,
                      dbgline, log->_msg, log->user);
