...
int aafi_retrieveData(AAF_Iface *aafi) {
  ...
  // {
  // 				/* loops through Mobs */
  // 				aafUID_t *UsageCode = aaf_get_propertyValue(
  // Mob, PID_Mob_UsageCode, &AAFTypeID_ );
  //
  // 				aafMobID_t *MobID = aaf_get_propertyValue( Mob,
  // PID_Mob_MobID, &AAFTypeID_MobIDType );
  //
  ...
  // 				debug( "Clip SourceID      : %ls",
  // aaft_MobIDToText(MobID) );
  //
  ...
  //
  // 				aafObject *MobSlots = aaf_get_propertyValue(
  // Mob, PID_Mob_Slots, &AAFTypeID_MobSlotStrongReferenceVector ); 				aafObject
  // *MobSlot = NULL; 				int SlotID = 1; 				aaf_foreach_ObjectInSet( &MobSlot,
  // MobSlots, NULL ) { 					debug( "  SlotID %u", SlotID );
  //
  // 					aafObject *Segment =
  // aaf_get_propertyValue( MobSlot, PID_MobSlot_Segment,
  // &AAFTypeID_SegmentStrongReference );
  //
  // 					if ( Segment == NULL ) {
  // 						error( "Missing
  // MobSlot::Segment." ); 						return -1;
  // 					}
  //
  //
  // 					aafUID_t *DataDefinition =
  // get_Component_DataDefinition( aafi, Segment );
  //
  // 					if ( DataDefinition == NULL ) {
  // 						error( "Could not retrieve
  // MobSlot::Segment DataDefinition." ); 						return -1;
  // 					}
  //
  // 					// CURRENTPOINTER
  //
  // 					debug( "    Segment : %ls",
  // aaft_ClassIDToText( aafi->aafd, Segment->Class->ID ) ); 					debug( "
  // DataDefinition : %ls", aaft_DataDefToText(aafi->aafd, DataDefinition) );
  //
  //
  //
  //
  //
  //
  // 					if ( aafUIDCmp( Segment->Class->ID,
  // &AAFClassID_SourceClip ) ) { 						aafMobID_t *SourceID = aaf_get_propertyValue(
  // Segment, PID_SourceReference_SourceID, &AAFTypeID_MobIDType ); 						debug( "
  // SourceID : %ls", aaft_MobIDToText(sourceID) );
  // 					}
  //
  // 					SlotID++;
  // 				}
  ...
  // 		 *
  // 		 * NOTE We do it after the main loop, so we make sure all
  // MasterMobs was parsed.
  // 		 */
  //
  ...
  //
  // 		if ( audioEssence == NULL ) {
  // 			aafi_trace_obj( aafi, aafi->ctx.MobSlot,
  // ANSI_COLOR_YELLOW ); 			debug( "%ls", aaft_DataDefToText( aafi->aafd,
  // DataDefinition ) );
  // 		}
  //
  // 	}
