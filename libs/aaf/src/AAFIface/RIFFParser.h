...
struct RIFFAudioFile {
  ...
  uint16_t sampleSize;
  uint16_t channels;
  uint64_t duration; /* total samples for 1 channel (no matter channel count).
                        (duration / sampleRate) = duration in seconds */
};
...
PACK(struct wavBextChunk {
   ...
   */
  int16_t loudness_value;      // 0xd8f1 - 0xffff (-99.99 -0.1) and 0x000 0x270f
                               // (0.00 99.99)
  int16_t loudness_range;      // 0x0000 - 0x270f (0.00  99.99)
  int16_t max_true_peak_level; // 0xd8f1 - 0xffff (-99.99 -0.1) and 0x000 0x270f
                               // (0.00 99.99)
  int16_t max_momentary_loudness;  // 0xd8f1 - 0xffff (-99.99 -0.1) and 0x000
                                   // 0x270f (0.00 99.99)
  int16_t max_short_term_loudness; // 0xd8f1 - 0xffff (-99.99 -0.1) and 0x000
                                   // 0x270f (0.00 99.99)
...
PACK(struct aiffCOMMChunk {
  ...
  uint16_t sampleSize;
  unsigned char
      sampleRate[10]; // 80 bit IEEE Standard 754 floating point number
});
...
int riff_parseAudioFile(struct RIFFAudioFile *RIFFAudioFile,
                        ...
                        size_t (*readerCallback)(unsigned char *, size_t,
                                                 size_t, void *, void *,
                                                 void *),
                        void *user1, void *user2, void *user3, struct dbg *dbg);
