 ...
 * @addtogroup LibCFB
 * @{
 * @brief LibCFB is a library that allows to parse any Compound File Binary File
 * Format (a.k.a *Structured Storage File Format*).
 *
 * The specifications of the CFB can be found at
 * https://www.amwa.tv/projects/MS-03.shtml
 *
 * @note When using LibAAF, you should not have to talk to LibCFB directly.\n
 * All the following steps are handled by LibAAF.
 *
 * Interaction with LibCFB is done through the CFB_Data structure, which is
 * allocated with cfb_alloc().
 *
 * The first thing you will need to do in order to parse any CFB file is to
 * allocate CFB_Data with cfb_alloc(). Then, you can "load" a file by calling
 * cfb_load_file(). It will open the file for reading, check if it is a regular
 * CFB file, then retrieve all the CFB components (Header, DiFAT, FAT, MiniFAT)
 ...
 * cfb_getNodeByPath() and cfb_getChildNode(), and access a node's stream with
 * the functions cfb_getStream() or cfb_foreachSectorInStream(). The former is
 * prefered for small-size streams, since it allocates the entire stream to
 * memory, while the later loops through each sector that composes a stream,
 * better for the big ones.
 *
 * **Example:**
 * @code
 ...
 *
 *
 * // Get a "file" (stream node) called "properties" inside the "Header"
 * directory cfbNode *properties = cfb_getNodeByPath( cfbd,
 * "/Header/properties", 0 );
 *
 *
 ...
 * 	@endcode
 *
 * Finaly, once you are done working with the file, you can close the file and
 * free the CFB_Data and its content by simply calling cfb_release().
 *
 * **Example:**
 * @code
 * cfb_release( &cfbd );
...
static uint64_t cfb_readFile(CFB_Data *cfbd, unsigned char *buf,
...
const wchar_t *cfb_CLSIDToText(const cfbCLSID_t *clsid) {
  ...
  if (clsid == NULL) {
    ...
    swprintf(str, sizeof(str),
             L"{ 0x%08x 0x%04x 0x%04x { 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x "
...
static int cfb_is_valid(CFB_Data *cfbd) {
  ...
  if (cfb_readFile(cfbd, (unsigned char *)&signature, 0, sizeof(uint64_t)) !=
      sizeof(uint64_t)) {
    return 0;
  }
...
static uint64_t cfb_readFile(CFB_Data *cfbd, unsigned char *buf,
  ...
  if (len + offset > cfbd->file_sz) {
    error("Requested data goes %" PRIu64
          " bytes beyond the EOF : offset %" PRIu64 " | length %" PRIu64 "",
          (len + offset) - cfbd->file_sz, offset, len);
    return 0;
  }
  ...
  if (byteRead < len) {
    warning("Could only retrieve %" PRIu64 " bytes out of %" PRIu64
            " requested.",
            byteRead, len);
  }
...
unsigned char *cfb_getSector(CFB_Data *cfbd, cfbSectorID_t id) {
  ...
  if (cfbd->fat_sz > 0 && id >= cfbd->fat_sz) {
    error("Asking for an out of range FAT sector @ index %u (max FAT index is "
          "%u)",
          id, cfbd->fat_sz);
    return NULL;
  }
...
unsigned char *cfb_getMiniSector(CFB_Data *cfbd, cfbSectorID_t id) {
  ...
  if (cfbd->fat_sz > 0 && id >= cfbd->miniFat_sz) {
    error("Asking for an out of range MiniFAT sector @ index %u (0x%x) "
          "(Maximum MiniFAT index is %u)",
          id, id, cfbd->miniFat_sz);
    return NULL;
  }
  ...
  /* Fat Divisor: allow to guess the number of mini-stream sectors per standard
   * sector */
  ...
  *stream = calloc(stream_len, sizeof(unsigned char));
  ...
  if (stream_len < cfbd->hdr->_ulMiniSectorCutoff) { /* mini-stream */
    ...
    cfb_foreachMiniSectorInChain(cfbd, buf, id) {
      ...
      cpy_sz = ((stream_len - offset) <
                (uint64_t)(1 << cfbd->hdr->_uMiniSectorShift))
                   ? (stream_len - offset)
                   : (uint64_t)(1 << cfbd->hdr->_uMiniSectorShift);
    ...
    cfb_foreachSectorInChain(cfbd, buf, id) {
      ...
      cpy_sz =
          ((stream_len - offset) < (uint64_t)(1 << cfbd->hdr->_uSectorShift))
              ? (stream_len - offset)
              : (uint64_t)(1 << cfbd->hdr->_uSectorShift);
 ...
 * @param  buf       Pointer to pointer to the buffer that will receive the
 * sector's content.
 * @param  bytesRead Pointer to a size_t that will receive the number of bytes
 * retreived. This should be equal to the sectorSize, except for the last sector
...
static int cfb_retrieveFileHeader(CFB_Data *cfbd) {
  ...
  if (cfb_readFile(cfbd, (unsigned char *)cfbd->hdr, 0, sizeof(cfbHeader)) ==
      0) {
    free(cfbd->hdr);
    cfbd->hdr = NULL;
    return -1;
...
static int cfb_retrieveDiFAT(CFB_Data *cfbd) {
  ...
  if (cfbd->hdr->_csectFat > 109) {
    csectDif = ceil((float)((cfbd->hdr->_csectFat - 109) * 4) /
                    (1 << cfbd->hdr->_uSectorShift));
  }

  if (csectDif != cfbd->hdr->_csectDif) {
    warning("cfbd->hdr->_csectDif value seems wrong (%u)",
            cfbd->hdr->_csectDif);
    // warning( "cfbd->hdr->_csectDif value seems wrong (%u). Correcting from
    // cfbd->hdr->_csectFat.", cfbd->hdr->_csectDif ); cfbd->hdr->_csectDif =
    // csectDif;
  }

  if (csectDif == 0 && cfbd->hdr->_sectDifStart != CFB_END_OF_CHAIN) {
    warning("cfbd->hdr->_sectDifStart is 0x%08x (%u) but should be "
            "CFB_END_OF_CHAIN. Correcting.",
  ...
  cfb_foreachSectorInDiFATChain(cfbd, buf, id) {
    ...
    if (buf == NULL) {
      error("Error retrieving sector %u (0x%08x) out of the DiFAT chain.", id,
            id);
      return -1;
    }

    memcpy((unsigned char *)DiFAT + offset, buf,
           (1 << cfbd->hdr->_uSectorShift) - 4);
...
static int cfb_retrieveFAT(CFB_Data *cfbd) {
  ...
  uint64_t FAT_sz =
      (((cfbd->hdr->_csectFat) * (1 << cfbd->hdr->_uSectorShift))) /
  ...
  cfb_foreachFATSectorIDInDiFAT(cfbd, id) {
    ...
    if (cfbd->DiFAT[id] == 0x00000000 && id > 0) {
      warning(
          "Got a NULL FAT index in the DiFAT @ %u, should be CFB_FREE_SECT.",
          id);
      continue;
    }
    ...
    memcpy(((unsigned char *)FAT) + offset, buf,
           (1 << cfbd->hdr->_uSectorShift));
...
static int cfb_retrieveMiniFAT(CFB_Data *cfbd) {
  ...
  uint64_t miniFat_sz = cfbd->hdr->_csectMiniFat *
                        (1 << cfbd->hdr->_uSectorShift) / sizeof(cfbSectorID_t);
  ...
  cfb_foreachSectorInChain(cfbd, buf, id) {
    ...
    memcpy((unsigned char *)miniFat + offset, buf,
           (1 << cfbd->hdr->_uSectorShift));
...
static int cfb_retrieveNodes(CFB_Data *cfbd) {
  ...
  if (cfbd->hdr->_uSectorShift == 9) { /* 512 bytes sectors */
    ...
    warning("Parsing non-standard sector size !!! (%u bytes)",
            (1 << cfbd->hdr->_uSectorShift)) uint32_t nodesPerSect =
        (1 << cfbd->hdr->_uMiniSectorShift) / sizeof(cfbNode);
 ...
 *
 * @param buf      Pointer to wchar_t output buffer. If NULL, then function will
 * allocate a new buffer.
 * @param w16buf   Pointer to a 16-bits MS/CFB "wchar_t" array.
 * @param w16blen  Size of the w16buf array in bytes, including the NULL. If it
 * is set to CFB_W16TOWCHAR_STRLEN, then function will parse w16buf up to NULL
 * to retrieve w16buf byte size
 *
 ...
 *
 * @param cfbd      Pointer to the CFB_Data structure.
 * @param path      Pointer to a NULL terminated char array, holding the Node
 * path.
 * @param id        Next node index. This is used internaly by the function and
 * should be set to 0 when called by the user.
 *
