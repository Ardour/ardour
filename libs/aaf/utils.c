...
static int utf8CodeLen(const uint16_t *u16Code);
static int utf16CodeLen(const uint16_t *u16Code);
static int utf16CodeToUTF8(const uint16_t *u16Code, char *u8Code, int *u16Len,
                           int *u8Len);
static long utf8strLen(const uint16_t *u16str);
  ...
  do {
    ...
    int written = snprintf(
        ...
        ((element_count == 0 && has_leading_sep) || (element_count > 0)) ? sep
                                                                         : "",
        (uint32_t)(arglen - argstart), arg + argstart);
...
char *laaf_util_relative_path(const char *filepath, const char *refpath) {
  ...
  if (isWindowsPath == 1) {
    // fprintf( stderr, "Trying to make a relative path out of a windows path
    // and a non-windows path\n" );
    return NULL;
  }
  ...
  if (strncmp(filepath + winDriveLetterOffset, refpath + winDriveLetterOffset,
              strlen(refpath)) == 0) {
  ...
  char *_refpath = laaf_util_build_path(
      "/", refpath, "/", NULL); /* ensures there is always a trailing '/' */
  ...
  while (p > (_refpath + winDriveLetterOffset)) {
    ...
    if (strncmp(_filepath + winDriveLetterOffset,
                _refpath + winDriveLetterOffset,
                strlen(_refpath + winDriveLetterOffset)) == 0) {
      if (!parents) {
        relpath =
            laaf_util_build_path("/", "./", _filepath + strlen(_refpath), NULL);
        goto end;
      } else {
        relpath = laaf_util_build_path("/", parents,
                                       _filepath + strlen(_refpath), NULL);
        goto end;
      }
    }

    int ret =
        laaf_util_snprintf_realloc(&parents, &parentsLen, parentsOffset, "../");
...
int laaf_util_snprintf_realloc(char **str, size_t *size, size_t offset,
  ...
  while (0 <= (retval =
         ...
         ((*size) - offset) < (needed = (unsigned)retval + 1)) {
    va_end(ap);
...
int laaf_util_dump_hex(const unsigned char *stream, size_t stream_sz,
  ...
  int rc = laaf_util_snprintf_realloc(buf, bufsz, offset,
                                      "%s______________________________ Hex "
  ...
  while (count < stream_sz) {
    ...
    for (i = 0; i < lineLen; i++) {
      ...
      rc = snprintf(&hex[linepos], sizeof(hex) - (linepos), "%02x%s",
  ...
  rc = laaf_util_snprintf_realloc(buf, bufsz, offset,
