...
public:
	...
	*/
	...
	bool export_track_state (std::shared_ptr<RouteList> rl, const std::string& path, bool with_sources);
	int  import_track_state (const std::string& path, std::map<PBD::ID, PBD::ID> const&);

	std::map<PBD::ID, std::string> parse_track_state (const std::string& path, bool& match_pbd_id);
