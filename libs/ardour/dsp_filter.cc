...
PerceptualAnalyzer::Trace::~Trace (void)
{
	delete[] _data;
}

PerceptualAnalyzer::PerceptualAnalyzer (double fsamp, int ipsize, int fftmax)
	: _ipsize (ipsize)
	, _icount (0)
	, _fftmax (fftmax)
	, _fftlen (0)
...
PerceptualAnalyzer::~PerceptualAnalyzer (void)
{
	if (_fftplan) {
		Glib::Threads::Mutex::Lock lk (ARDOUR::fft_planner_lock);
		fftwf_destroy_plan (_fftplan);
...
void
PerceptualAnalyzer::set_fftlen (int fftlen)
{
	if (fftlen > _fftmax) {
		fftlen = _fftmax;
	}
...
void
PerceptualAnalyzer::clr_peak (void)
{
	_peakp->_valid = false;
	_peakp->_count = 0;
	memset (_peakp->_data,  0, (_fftlen + 1) * sizeof (float));
...
void
PerceptualAnalyzer::ipskip (int iplen)
{
	_icount += iplen;
	if (_icount >= _ipsize)
		_icount -= _ipsize;
...
{
	assert (bin < _fftmax);
	const double f = 0.5 * bin / _fftmax;
	// TODO cache
	return warp_freq (-_wfact, f) * _fsamp;
}

float
PerceptualAnalyzer::power_at_bin (const uint32_t b) const
{
	assert (b < _fftmax);
	// TODO apply warp_freq bw correction
	return log10f (_power->_data[b] + 1e-30);
}
