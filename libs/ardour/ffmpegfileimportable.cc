	...
	, _ffmpeg_exec (0)
{
	...
	try {
		...
		exec->ReadStdout.connect_same_thread (c, std::bind (&receive_stdout, &ffprobe_output, _1, _2));
...
FFMPEGFileImportableSource::start_ffmpeg ()
{
	...
	_ffmpeg_exec->ReadStdout.connect_same_thread (_ffmpeg_conn, std::bind (&FFMPEGFileImportableSource::did_read_data, this, _1, _2));
}
