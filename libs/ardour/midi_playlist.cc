		...
		for (auto i = regs.rbegin(); i != regs.rend(); ++i) {
			...
			if (top) {
				...
				for (auto & ev : tmp) {
					...
					} else if ((3 == ev->size()) && (t.samples() == mr->end_position().samples())) {

						/* this event is not considered "audible" at this time, But it could be a note-off
						   generated by the region's own note resolving, which would be placed at the end_position()
						   of the region, and is still considered part of the relevant events.
						*/

						uint8_t* evbuf = ev->buffer();
						if ((evbuf[0] & 0xf0) == MIDI_CMD_NOTE_OFF && mtr.active (evbuf[1], evbuf[0] & 0x0f)) {
							DEBUG_TRACE (DEBUG::MidiPlaylistIO, string_compose ("merge resolved note off event %1 region end %2\n", *ev, mr->end_position().samples()));
							evlist.write (ev->time(), ev->event_type(), ev->size(), evbuf);
							mtr.track (evbuf);
						} else {
							DEBUG_TRACE (DEBUG::MidiPlaylistIO, string_compose ("ignored event %1\n", *ev));
						}
					} else {
