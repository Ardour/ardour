...
RegionFxPlugin::RegionFxPlugin (Session& s, std::shared_ptr<Plugin> plug)
...
RegionFxPlugin::configure_io (ChanCount in, ChanCount out)
{
	...
	switch (_match.method) {
		...
		case Delegate: {
			...
			assert (r);
			return _plugins.front ()->reconfigure_io (din, daux, dout);
		}
		case Replicate:
	...
	pc = 0;
	std::cout << "\n--------<<--------\n";
	std::cout << string_compose ("RFX IO Config for %1 in: %2 out: %3 req: %4\n", name(), _configured_in, _configured_out, _required_buffers);
	std::cout << "Match: " << _match << " no inplace: " << _no_inplace << "\n";
	for (Plugins::iterator i = _plugins.begin(); i != _plugins.end(); ++i, ++pc) {
		if (pc > 0) {
			std::cout << "----><----\n";
		}
		std::cout << string_compose ("Channel Map for %1 plugin %2\n", name(), pc);
		std::cout << " * Inputs:\n";
		std::cout << _in_map[pc];
		std::cout << " * Outputs:\n";
		std::cout << _out_map[pc];
	}
	std::cout << "-------->>--------\n";
#endif
