...
Session::replace_region_source_command_factory (XMLNode* n)
{
	...
	if (!n->get_property (X_("playlist-id"), playlist_id) ||
	    ...
	    !n->get_property (X_("position-samples"), position_samples)) {
	...
	ReplaceRegionSourceCommand* cmd = new ReplaceRegionSourceCommand (playlist, original, replacement, timepos_t (position_samples));
