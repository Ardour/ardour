...
bool PortAudioBackend::is_realtime() const
{
	return true;
}

bool PortAudioBackend::requires_driver_selection() const
{
...
int PortAudioBackend::set_driver(const std::string &name)
{
...
bool PortAudioBackend::update_devices()
{
	// update midi device info?
	return _pcmio->update_devices();
}

void PortAudioBackend::set_use_buffered_io(bool use_buffered_io)
{
...
bool PortAudioBackend::use_separate_input_and_output_devices() const
{
	return true;
}
...
bool PortAudioBackend::can_change_sample_rate_when_running() const
{
	return false;
}

bool PortAudioBackend::can_change_buffer_size_when_running() const
{
	return false; // TODO
}

int PortAudioBackend::set_device_name(const std::string &d)
{
	DEBUG_AUDIO("Portaudio: set_device_name should not be called\n");
	return 0;
}

int PortAudioBackend::set_input_device_name(const std::string &d)
{
...
int PortAudioBackend::set_output_device_name(const std::string &d)
{
...
int PortAudioBackend::set_sample_rate(float sr)
{
	if (sr <= 0)
	{
		return -1;
...
int PortAudioBackend::set_buffer_size(uint32_t bs)
{
...
int PortAudioBackend::set_interleaved(bool yn)
{
	if (!yn)
	{
		return 0;
...
int PortAudioBackend::set_systemic_input_latency(uint32_t sl)
{
	_systemic_audio_input_latency = sl;
	return 0;
}

int PortAudioBackend::set_systemic_output_latency(uint32_t sl)
{
	_systemic_audio_output_latency = sl;
	return 0;
}

int PortAudioBackend::set_systemic_midi_input_latency(std::string const device, uint32_t sl)
{
#ifdef _WIN32
...
int PortAudioBackend::set_systemic_midi_output_latency(std::string const device, uint32_t sl)
{
#ifdef _WIN32
	MidiDeviceInfo *nfo = midi_device_info(device);
	if (!nfo)
...
float PortAudioBackend::sample_rate() const
{
	return _samplerate;
}
...
bool PortAudioBackend::interleaved() const
{
	return false;
}
...
void PortAudioBackend::launch_control_app()
{
...
int PortAudioBackend::set_midi_option(const std::string &opt)
{
...
int PortAudioBackend::set_midi_device_enabled(std::string const device, bool enable)
{
#ifdef _WIN32
	MidiDeviceInfo *nfo = midi_device_info(device);
	if (!nfo)
...
bool PortAudioBackend::midi_device_enabled(std::string const device) const
{
#ifdef _WIN32
	MidiDeviceInfo *nfo = midi_device_info(device);
	if (!nfo)
...
bool PortAudioBackend::engine_halted()
{
	return !_active && _run;
}

bool PortAudioBackend::running()
{
	return _active || _run;
}
...
int PortAudioBackend::portaudio_callback(const void *input,
										 void *output,
...
bool PortAudioBackend::process_callback(const float *input,
										float *output,
...
bool PortAudioBackend::start_blocking_process_thread()
{
...
bool PortAudioBackend::stop_blocking_process_thread()
{
	void *status;
...
bool PortAudioBackend::start_freewheel_process_thread()
{
...
bool PortAudioBackend::stop_freewheel_process_thread()
{
	void *status;
...
int PortAudioBackend::freewheel(bool onoff)
{
...
float PortAudioBackend::dsp_load() const
{
...
int PortAudioBackend::name_to_id(std::string device_name) const
{
...
#ifdef _WIN32
bool PortAudioBackend::set_mmcss_pro_audio(HANDLE *task_handle)
{
...
bool PortAudioBackend::reset_mmcss(HANDLE task_handle)
{
...
#else
	DEBUG_THREADS("Process Thread Child ID: (non-Windows)\n");
#endif
...
int PortAudioBackend::create_process_thread(std::function<void()> func)
{
	pthread_t thread_id;
...
int PortAudioBackend::join_process_threads()
{
	int rv = 0;
...
bool PortAudioBackend::in_process_thread()
{
	if (_use_blocking_api)
	{
...
void PortAudioBackend::update_latencies()
{
...
int PortAudioBackend::register_system_audio_ports()
{
	LatencyRange lr;
...
int PortAudioBackend::register_system_midi_ports(std::string const &device)
{
#ifdef _WIN32
	LatencyRange lr;

#ifdef _WIN32
	std::vector<WinMMEMidiInputDevice *> inputs = _midiio->get_inputs();
#endif
...
#ifdef _WIN32
	std::vector<WinMMEMidiOutputDevice *> outputs = _midiio->get_outputs();
#endif
...
void PortAudioBackend::update_systemic_midi_latencies()
{
#ifdef _WIN32
...
/* MIDI */
int PortAudioBackend::midi_event_get(
	pframes_t &timestamp,
...
int PortAudioBackend::midi_event_put(
	void *port_buffer,
	pframes_t timestamp,
	const uint8_t *buffer, size_t size)
{
	if (!buffer || !port_buffer)
		return -1;
...
void PortAudioBackend::midi_clear(void *port_buffer)
{
	if (!port_buffer)
		return;
...
bool PortAudioBackend::can_monitor_input() const
{
	return false;
}

int PortAudioBackend::request_input_monitoring(PortEngine::PortHandle, bool)
{
	return -1;
}

int PortAudioBackend::ensure_input_monitoring(PortEngine::PortHandle, bool)
{
	return -1;
}

bool PortAudioBackend::monitoring_input(PortEngine::PortHandle)
{
	return false;
}
...
void PortAudioBackend::set_latency_range(PortEngine::PortHandle port_handle, bool for_playback, LatencyRange latency_range)
{
...
bool PortAudioBackend::blocking_process_main(const float *interleaved_input_data,
...
bool PortAudioBackend::blocking_process_freewheel()
{
...
void PortAudioBackend::process_incoming_midi()
{
#ifdef _WIN32
	uint32_t i = 0;
...
void PortAudioBackend::process_outgoing_midi()
{
#ifdef _WIN32
	/* mixdown midi */
...
PortMidiPort::PortMidiPort(PortAudioBackend &b, const std::string &name, PortFlags flags)
	: BackendPort(b, name, flags), _n_periods(1), _bufperiod(0)
{
	_buffer[0].clear();
	_buffer[1].clear();
...
#else
	if (size > 0 && size < 4) // stub size for non-Windows
#endif
	{
...
PortMidiEvent::PortMidiEvent(const PortMidiEvent &other)
	: _size(other.size()), _timestamp(other.timestamp())
{
	if (other._size > 0)
	{
#ifdef _WIN32
...
#else
		assert(other._size < 4); // stub size for non-Windows
#endif
