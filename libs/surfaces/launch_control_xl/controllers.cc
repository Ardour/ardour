...
LaunchControlXL::build_maps ()
{
	...
	if (!device_mode()) {	/* mixer mode */
		MAKE_KNOB (SendA1, 13, 0, RedFull, RedLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 0)));
		MAKE_KNOB (SendA2, 14, 1, YellowFull, YellowLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 1)));
		MAKE_KNOB (SendA3, 15, 2, GreenFull, GreenLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 2)));
		MAKE_KNOB (SendA4, 16, 3, AmberFull, AmberLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 3)));
		MAKE_KNOB (SendA5, 17, 4, RedFull, RedLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 4)));
		MAKE_KNOB (SendA6, 18, 5, YellowFull, YellowLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 5)));
		MAKE_KNOB (SendA7, 19, 6, GreenFull, GreenLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 6)));
		MAKE_KNOB (SendA8, 20, 7, AmberFull, AmberLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendA, this, 7)));

		MAKE_KNOB (SendB1, 29, 8, RedFull, RedLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 0)));
		MAKE_KNOB (SendB2, 30, 9, YellowFull, YellowLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 1)));
		MAKE_KNOB (SendB3, 31, 10, GreenFull, GreenLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 2)));
		MAKE_KNOB (SendB4, 32, 11, AmberFull, AmberLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 3)));
		MAKE_KNOB (SendB5, 33, 12, RedFull, RedLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 4)));
		MAKE_KNOB (SendB6, 34, 13, YellowFull, YellowLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 5)));
		MAKE_KNOB (SendB7, 35, 14, GreenFull, GreenLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 6)));
		MAKE_KNOB (SendB8, 36, 15, AmberFull, AmberLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_sendB, this, 7)));

		MAKE_KNOB (Pan1, 49, 16, RedFull, RedLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 0)));
		MAKE_KNOB (Pan2, 50, 17, YellowFull, YellowLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 1)));
		MAKE_KNOB (Pan3, 51, 18, GreenFull, GreenLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 2)));
		MAKE_KNOB (Pan4, 52, 19, AmberFull, AmberLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 3)));
		MAKE_KNOB (Pan5, 53, 20, RedFull, RedLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 4)));
		MAKE_KNOB (Pan6, 54, 21, YellowFull, YellowLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 5)));
		MAKE_KNOB (Pan7, 55, 22, GreenFull, GreenLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 6)));
		MAKE_KNOB (Pan8, 56, 23, AmberFull, AmberLow, std::function<void ()> (boost::bind (&LaunchControlXL::knob_pan, this, 7)));
		...
		for (uint8_t i = 0; i < 8; ++i) {
			MAKE_DM_KNOB (static_cast<KnobID>(i + 16), (i + 49), (i + 16), GreenLow, YellowLow, std::function<void()>
				...
				std::function<uint8_t ()> (boost::bind(&LaunchControlXL::dm_mb_check_send_knob, this, static_cast<KnobID>(i + 16))));
		}
		...
		if (first_selected_stripable() && (first_selected_stripable()->is_master() || first_selected_stripable()->mixbus())) {
			...
			MAKE_DM_KNOB (SendA8, 20, 7, YellowLow, AmberLow, std::function<void ()>
			              ...
			              std::function<uint8_t ()> (boost::bind(&LaunchControlXL::dm_mb_eq_gain_enabled, this, 3)));
		}
	...
	if (!device_mode()) {	/* mixer mode */
		for (uint8_t i = 0; i < 4; ++i) {
			MAKE_TRACK_BUTTON_PRESS(static_cast<ButtonID>(i), i+41, i+24, YellowFull, YellowLow,
				std::function<void()>	(boost::bind (&LaunchControlXL::button_track_focus, this, i)),
				std::function<uint8_t()> (boost::bind (&LaunchControlXL::dm_check_dummy, this, dev_nonexistant)));
		}

		for (uint8_t i = 0; i < 4; ++i) {
			MAKE_TRACK_BUTTON_PRESS(static_cast<ButtonID>(i+4), i+57, i+28, YellowFull, YellowLow,
				std::function<void()>	(boost::bind (&LaunchControlXL::button_track_focus, this, i+4)),
				std::function<uint8_t()> (boost::bind (&LaunchControlXL::dm_check_dummy, this, dev_nonexistant)));
		}
		...
		for (uint8_t i = 0; i < 4; ++i) {
			MAKE_TRACK_BUTTON_PRESS(static_cast<ButtonID>(i), i+41, i+24, GreenFull, YellowLow,
				std::function<void()>	(boost::bind (&LaunchControlXL::dm_mb_send_switch, this, static_cast<ButtonID>(i))),
				std::function<uint8_t()> (boost::bind (&LaunchControlXL::dm_mb_check_send_button, this, i)));

		}

		for (uint8_t i = 0; i < 4; ++i) {
			MAKE_TRACK_BUTTON_PRESS(static_cast<ButtonID>(i+4), i+57, i+28, GreenFull, YellowLow,
				std::function<void()>	(boost::bind (&LaunchControlXL::dm_mb_send_switch, this, static_cast<ButtonID>(i+4))),
				std::function<uint8_t()> (boost::bind (&LaunchControlXL::dm_mb_check_send_button, this, i+4)));
		}
#endif
	}
	...
	if (!device_mode()) {	/* mixer mode */
		...
		for (uint8_t i = 0; i < 4; ++i) {
			MAKE_CTRL_BUTTON_PRESS(static_cast<ButtonID>(i+8), i+73, i+32, Off, Off,
				std::function<void()>	(boost::bind (&LaunchControlXL::button_press_track_control, this, i)),
				std::function<uint8_t()> (boost::bind (&LaunchControlXL::dm_check_dummy, this, dev_nonexistant)));
		}

		for (uint8_t i = 0; i < 4; ++i) {
			MAKE_CTRL_BUTTON_PRESS(static_cast<ButtonID>(i+12), i+89, i+36, Off, Off,
				std::function<void()>	(boost::bind (&LaunchControlXL::button_press_track_control, this, i+4)),
				std::function<uint8_t()> (boost::bind (&LaunchControlXL::dm_check_dummy, this, dev_nonexistant)));
		}
		...
		MAKE_CTRL_BUTTON_PRESS(Control4, 76, 35, AmberFull, AmberLow,
			std::function<void()> (boost::bind (&LaunchControlXL::dm_mb_eq_shape_switch, this, 0)),
			std::function<uint8_t()> (boost::bind(&LaunchControlXL::dm_mb_eq_shape_enabled, this, 0 )));
		MAKE_CTRL_BUTTON_PRESS(Control5, 89, 36, AmberFull, AmberLow,
			std::function<void()> (boost::bind (&LaunchControlXL::dm_mb_eq_shape_switch, this, 3)),
			std::function<uint8_t()> (boost::bind(&LaunchControlXL::dm_mb_eq_shape_enabled, this, 3 )));
	...
	MAKE_SELECT_BUTTON_PRESS(SelectUp, 104, 44, std::function<void()> (boost::bind (&LaunchControlXL::send_bank_switch, this, false)));
	MAKE_SELECT_BUTTON_PRESS(SelectDown, 105, 45,  std::function<void()> (boost::bind (&LaunchControlXL::send_bank_switch, this, true)));
