...
OSCGlobalObserver::send_record_state_changed ()
{
	...
	_osc.float_message (X_("/is_recording"), (int)session->actively_recording (), addr);
	_osc.float_message (X_("/has_rec_enabled_track"), (int)session->have_rec_enabled_track (), addr);
