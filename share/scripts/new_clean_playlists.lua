ardour { ["type"] = "EditorAction", name = "New clean playlists",
	...
	description = [[Copy the current playlist of selected audio tracks to a new playlist with audible regions only.]]
}
	...
	for route in sel.tracks:routelist():iter() do
		...
		for r in playlist:region_list():iter() do
			...
			for rg in playlist:region_list():iter() do
				...
				local cut_point_left = rg:position():samples()
				local cut_point_right = cut_point_left + rg:length():samples()
		...
		for i=1, #regions_to_modify do
			...
			if #segments == 0 then
				...
				for j=1, #segments do
					...
					if segments[j][1] ~= rg:position():samples() then
						rg:cut_front(Temporal.timepos_t(segments[j][1]), 0)
					end

					if segments[j][2] ~= rg:position():samples() + rg:length():samples() then
						rg:cut_end(Temporal.timepos_t(segments[j][2]), 0)
					end
				end
			end
		end
