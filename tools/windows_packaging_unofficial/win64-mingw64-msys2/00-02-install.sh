...
#
# Compile, and install final dependencies.

export ARG1="${ARG1:-$1}"

install()
{
	echo "INFO: Starting install action"
	...
	for i in "${INSTALL_CMD[@]}"; do
		echo "INFO: Running install command: $i"
		$i
		echo "INFO: Install command succeeded: $i"
	done
	
	echo "INFO: Install commands finished successfully"
}
